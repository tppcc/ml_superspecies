...
MODULE mo_advection_hflux
  ...
  USe mo_advection_ssai,      ONLY: ssai      ! Reference to SSAI
...
CONTAINS
  ...
  SUBROUTINE hor_upwind_flux( p_cc, p_rhodz_now, p_rhodz_new, p_mass_flx_e, p_vn,   &
    &                         p_dtime, p_patch, p_int, p_upflux, opt_rlend, ssai_model)
    ...
    TYPE(ssai), INTENT(IN)  ::  ssai_model      ! Initialise SSAI Model object for further processing
    REAL(wp),TARGET, INTENT(IN) :: trLatent(:,:,:,:)  ! Latent representation of the tracer
...
!DIR$ ATTRIBUTES ALIGN : 64 :: z_real_vt
    ...
    ! Call projection to latent representation of the tracer field
    CALL ssai_model%projection(trAdvect%list(nt), trLatent)

    DO nt = 1, ssai%nst     ! Super tracer loop
      jt = compressed%list(nt)    ! Point to compressed superspecies
      !jt = trAdvect%list(nt)
...
! NOTE: this is only for testing; use upwind_hflux_miura/miura3 for performance
      ...
      CALL ssai_model%reconstruction()
