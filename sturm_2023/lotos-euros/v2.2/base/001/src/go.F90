...
!     with fields:
!     \bv
!       character(len=4)   ::  calendar     ! see 'calendars'
...
!     \begin{itemize}
!       \item 'greg'
!         The Gregorian calendar we are used to for a few ages now. \newline
!         Some years have a Februari 29. \newline
!         This is the default calendar type; see also 'Defaults' below.
!       \item '366'
...
!         Use this calendar if only operations on years and months are required.
!       \item 'wall'
!         Wall clock time, including time zone information.
!         Most operations are not implemented for this calendar yet.
!     \end{itemize}
!
!
!
...
!     \bv
!       t = NewDate( year=2000, month=1, ..., calendar='greg' )
!     \ev
!     Interface:
!     \bv
!       function NewDate( year, month, day, hour, min, sec, mili, zone, calendar )
!         type(TDate)                              ::  NewDate
...
!     the result is of calendar type \texttt{'wall'}:
!     \bv
!       t = SystemDate()
!     \ev
!
...
!     dates to each other.
!     Both should be of same calendar type, unless one is an increment:
!     \bv
!        to =  t1 +  t2
!        to =  t  + dt
!       dto = dt1 + dt2
...
!     Both should be of same calendar type, unless the substracted date 
...
!     both should be of same calendar type:
!     \bv
!       t1 == t2
!       t1 >= t2
!       t1 >  t2
...
!     the calendar type. Also implemented for date increments.
!     \bv
...
!     Yet, only the calendar type might be set.
!     \bv
!       call goDateDefaults( [calendar='greg'] )
!     \ev
!
!
!   \subsection{Hackers only}
...
module GO
  ...
  character(len=*), parameter, private  ::  mname = 'GO'
...
contains
  ...
  subroutine GO_Init( status )
    ...
    character(len=*), parameter  ::  rname = mname//'/GO_Init'
    ...
    IF_NOTOK_RETURN(status=1)
    ! for the moment from root only:
    call lgr%Set( status, apply=goc%root )
    IF_NOTOK_RETURN(status=1)

    ! ok
    status = 0
    
  end subroutine GO_Init
  
  
  ! ***
  
  
  subroutine GO_Done( status )
    ...
    character(len=*), parameter  ::  rname = mname//'/GO_Done'
    ...
    call goc%Done( status )
    IF_NOTOK_RETURN(status=1)
    
    ! ok
    status = 0
    
  end subroutine GO_Done
