...
! NAME
!
!   LE_Output_vd  -  LOTOS-EUROS output of deposition velocities
!
!   Relevant variables:
!
...
module LE_Output_vd
  ...
  public  ::  LE_Output_vd_Init, LE_Output_vd_Done
  public  ::  LE_Output_vd_PutOut


  ! --- const ------------------------------

  character(len=*), parameter   ::  mname = 'LE_Output_vd'


  ! maximum number of supported data sets:
  integer, parameter  ::  ndat = 50
...
contains
  ...
  subroutine LE_Output_vd_Init( leo, rcF, rckey, typ, name, state, status )
    ...
    type(T_LE_Output_vd), intent(out)     ::  leo
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_vd_Init'
    ...
    n = n+1; call SetDat( LE_Dat(n), 'Ra'               , 's/m'                , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Ra_obs'           , 's/m'                , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'fstab_obs'        , '1'                  , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Rb'               , 's/m'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Rc_eff'           , 's/m'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Rc_tot'           , 's/m'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'vd'               , 'm/s'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'lufrac'           , '-'                  , 2, .true. , .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'fluxdry'          , 'umol m-2 s-1'       , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'fstom_o3_hr'      , 'umol m-2 s-1'       , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'area'             , 'm2'                 , 2, .true. , .false., .false.)
    n = n+1; call SetDat( LE_Dat(n), 'lai'              , 'm2 leaf m-2 surface', 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'sai'              , 'm2 leaf m-2 surface', 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'frac_sto_o3_lu'   , '-'                  , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'rbc_o3_lu'        , 's/m'                , 2, .false., .false., .true. )
    n = n+1; call SetDat( LE_Dat(n), 'czcanopytop_o3_lu', 'mol mol-1'          , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Cc_lu'            , 'ug m-3'             , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Fd_tot_lu'        , 'ug m-2 s-1'         , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Fd_sto_lu'        , 'ug m-2 s-1'         , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Fd_ext_lu'        , 'ug m-2 s-1'         , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Fd_soi_lu'        , 'ug m-2 s-1'         , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Cstom'            , 'ug m-3'             , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Cw'               , 'ug m-3'             , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'Csoil'            , 'ug m-3'             , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'gstom'            , 'm/s'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'gw'               , 'm/s'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'gsoil_eff'        , 'm/s'                , 2, .false., .true. , .true. )                         
    n = n+1; call SetDat( LE_Dat(n), 'Rs'               , 's/m'                , 2, .false., .true. , .true. )
    n = n+1; call SetDat( LE_Dat(n), 'vs'               , 'm/s'                , 2, .false., .true. , .false. )
    n = n+1; call SetDat( LE_Dat(n), 'ustar'            , 'm/s'                , 2, .false., .false., .true.  )
    ...
    call goMatchValues( selected_names, LE_Dat(1:n)%name, &
  ...
  end subroutine LE_Output_vd_Init


  ! ***


  subroutine LE_Output_vd_Done( leo, status )
    ...
    type(T_LE_Output_vd), intent(inout)   ::  leo
    integer, intent(out)                  ::  status

    ! --- const ----------------------------

    character(len=*), parameter   ::  rname = mname//'/LE_Output_vd_Done'
  ...
  end subroutine LE_Output_vd_Done


  ! ***


  subroutine LE_Output_vd_PutOut( leo, t, c, bud, status )
    ...
    use LE_Stability    , only : dhobs
    use LE_DryDepos     , only : spectype_depos
    use LE_DryDepos     , only : Rb
    use LE_DryDepos     , only : Rc_eff, Rc_total
    use LE_DryDepos     , only : Rs
    use LE_DryDepos     , only : vs
    use LE_DryDepos     , only : vd_lu
    use LE_DryDepos     , only : Cc_lu
    use LE_DryDepos     , only : Fdry_total_lu
    use LE_DryDepos     , only : Fdry_stom_lu
    use LE_DryDepos     , only : Fdry_w_lu
    use LE_DryDepos     , only : Fdry_soil_lu
    use LE_DryDepos     , only : Cstom
    use LE_DryDepos     , only : Cw
    use LE_DryDepos     , only : Csoil
    use LE_DryDepos     , only : gstom
    use LE_DryDepos     , only : gw
    use LE_DryDepos     , only : gsoil_eff
    use LE_DryDepos     , only : frac_sto_o3_lu
    use LE_DryDepos     , only : rbc_o3_lu
    use LE_DryDepos     , only : czcanopytop_o3_lu
    use LE_DryDepos     , only : mix2canopytop
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_vd_PutOut'
    ...
    integer               ::  cmode
#endif
    integer               ::  idat, itr, ilu, ilev, ilev1, iz
    integer               ::  varid
    ...
    real, pointer         ::  z0h_lu(:,:,:)      ! (lon,lat,lu)   
        ...
        do m = 1, leo%ntr
          ...
          do n = 1, leo%nlu
            ...
            select case ( trim(leo%name_dat(l)) )
              ...
              case ( 'ustar' )
                write (cf_long_name,'("Friction velocity above ", a)') trim(lu_name(ilu))
              case ( 'Ra' )
                write (cf_long_name,'("atmospheric resistance [z0,htop] above ",a)') trim(lu_name(ilu))
              case ( 'Ra_obs' )
                write (cf_long_name,'("atmospheric resistance [zobs,htop] above ",a)') trim(lu_name(ilu))
              case ( 'fstab_obs' )
                write (cf_long_name,'("stability function [zobs,htop] above ",a)') trim(lu_name(ilu))
              case ( 'Rb' )
                write (cf_long_name,'("laminair viscous boundary layer resistance above ",a," for ",a)') trim(lu_name(ilu)), trim(specname(itr))
              case ( 'Rc_eff' )
                write (cf_long_name,'("effective surface resistance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Rc_tot' )
                write (cf_long_name,'("total surface resistance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Rs' )
                write (cf_long_name,'("Sedimentation resistance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'vd' )
                write (cf_long_name,'("deposition velocity above ",a," for ",a)') trim(lu_name(ilu)), trim(specname(itr))
              case ( 'vs' )
                write (cf_long_name,'("sedimentation velocity for ",a)') trim(specname(itr))
              case ( 'lufrac' )
                write (cf_long_name,'("landuse fraction of ",a)') trim(lu_name(ilu))
              case ( 'fluxdry' )
                write (cf_long_name,'("drydeposition flux of ",a," over ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'fstom_o3_hr' )
                write (cf_long_name,'("stomatal dry deposition of ",a," over ", a)' ) trim(specname(itr)), trim(lu_name(ilu))
              case ( 'lai' )
                write (cf_long_name,'("leaf area index for ",a)') trim(lu_name(ilu))
              case ( 'sai' )
                write (cf_long_name,'("surface area index for ",a)') trim(lu_name(ilu))
              case ( 'frac_sto_o3_lu' )
                write (cf_long_name,'("stomatal contribution to total flux for ",a)') trim(lu_name(ilu))
              case ( 'rbc_o3_lu' )
                write (cf_long_name,'("total surface resistance for ",a)') trim(lu_name(ilu))
              case ( 'czcanopytop_o3_lu' )
                write (cf_long_name,'("concentrations of ",a," at canopy top for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Cc_lu' )
                write (cf_long_name,'("canopy compensation point of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Fd_tot_lu' )
                write (cf_long_name,'("total exchange flux of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Fd_sto_lu' )
                write (cf_long_name,'("stomatal exchange flux of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Fd_ext_lu' )
                write (cf_long_name,'("external exchange flux of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Fd_soi_lu' )
                write (cf_long_name,'("soil exchange flux of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Cw' )
                write (cf_long_name,'("external leaf surface compensation point of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Cstom' )
                write (cf_long_name,'("stomatal compensation point of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'Csoil' )
                write (cf_long_name,'("soil compensation point of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'gw' )
                write (cf_long_name,'("external leaf surface conductance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'gstom' )
                write (cf_long_name,'("stomatal conductance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case ( 'gsoil_eff' )
                write (cf_long_name,'("effective soil conductance of ",a," for ",a)') trim(specname(itr)), trim(lu_name(ilu))
              case default
...
!        if ( LE_Dat(idat)%name == 'czcanopytop_o3_lu' ) then
!          call mix2canopytop(c , czcanopytop_o3_lu(:,:,:), itr )
!        endif
          ...
          select case ( LE_Dat(idat)%rank )
            ...
            !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
              ...
              select case ( trim(LE_Dat(idat)%name) )
                ...
                ! deposition velocity (2-dimensional, used in the model for surface layer)
                case ( 'vd' )                
                  pat = vd_lu(:,:,itr,ilu)
                ...
                ! atmospheric resistance over [z0,htop]
                case ( 'Ra' )
                  pat = Ra_h_htop_z0_lu(:,:,ilu)
                ! atmospheric resistance over [zobs,htop]
                case ( 'Ra_obs' )
                  pat = Ra_h_htop_zobs_lu(:,:,ilu)
                
                ! stability function over [zobs,htop]
                case ( 'fstab_obs' )
                  ! pointers to data:
                  ...
                  IF_NOTOK_RETURN(status=1)
                  ! evaluate:
                  pat = f_h_stability( h_m(:,:,1), z0h_lu(:,:,ilu)+dhobs, monin_inv(:,:,1) )

                ! atmospheric resistance (2 dimensional, use lowest layer Ra)
                case ( 'ustar_lu' )
                  pat = ustar_lu(:,:,ilu)
                  
                ! viscous boundary layer resistance
                case ( 'Rb' )
                  ! obtain from deposition module:
                  pat = Rb(:,:,itr,ilu)
                ...
                case ( 'lai', 'sai' )
                  ! point to meteo data:
                  call LE_Data_GetPointer( trim(LE_Dat(idat)%name)//'_lu', pdata, status, check_units='m2/m2' )
                  IF_NOTOK_RETURN(status=1)
                  ! select:
                  pat(:,:) = pdata(1:nx,1:ny,ilu)

                ! fraction of stomatal flux to total flux
                case ( 'frac_sto_o3_lu' )
                  pat = frac_sto_o3_lu(:,:,ilu)

                ! total surface resistance (Rb+Rc) needed for C at canopy top
                case ( 'rbc_o3_lu' )
                  pat = rbc_o3_lu(:,:,ilu)

                ! concentration at canopy top per landuse class
                case ( 'czcanopytop_o3_lu' )
                  pat = czcanopytop_o3_lu(:,:,ilu)/1e9

                ! canopy compensation point per tracer per landuse class
                case ( 'Cc_lu' )
                  pat = Cc_lu(:,:,itr,ilu)

                ! total exchange flux per tracer per landuse class
                case ( 'Fd_tot_lu' )
                  pat = Fdry_total_lu(:,:,itr,ilu)

                ! stomatal exchange flux per tracer per landuse class
                case ( 'Fd_sto_lu' )
                  pat = Fdry_stom_lu(:,:,itr,ilu)

                ! external exchange flux per tracer per landuse class
                case ( 'Fd_ext_lu' )
                  pat = Fdry_w_lu(:,:,itr,ilu)

                ! soil exchange flux per tracer per landuse class
                case ( 'Fd_soi_lu' )
                  pat = Fdry_soil_lu(:,:,itr,ilu)

                ! external leaf surface compensation point per tracer per landuse class
                case ( 'Cw' )
                  pat = cw(:,:,itr,ilu)

                ! external leaf surface conductance per tracer per landuse class
                case ( 'gw' )
                  pat = gw(:,:,itr,ilu)

                ! stomatal compensation point per tracer per landuse class
                case ( 'Cstom' )
                  pat = cstom(:,:,itr,ilu)

                ! stomatal conductance per tracer per landuse class
                case ( 'gstom' )
                  pat = gstom(:,:,itr,ilu)

                ! soil compensation point per tracer per landuse class
                case ( 'Csoil' )
                  pat = csoil(:,:,itr,ilu)

                ! effective soil conductance per tracer per landuse class
                case ( 'gsoil_eff' )
                  pat = gsoil_eff(:,:,itr,ilu)

                case ( 'fluxdry' )
                  ! current budget:
                  pat = bud%ex_hour_lu(:,:,itr,ilu,ex_drydepo)
                  ! unit conversion; 'cdry' is 'concentation * height', 
                  ! thus 'ppb m' or 'ug/m3 m'
                  select case ( trim(specunit(itr))//' m -> '//trim(LE_Dat(idat)%unit) )
                  !~ other notations ...
                    ...
                    case ( 'ppb m -> ug m-2 h-1' )
                      ! index in density array
                      iz = 1
                      ! ug/m2        =      ppb  m    * ((mole tr/mole air)/ppb) * (kg tr/mole tr) *   (kg air/m3 air)  / (kg air/mole air) * ug/kg
                    ...
                    case ( 'ppb m -> kg m-2 s-1' )
                      ! index in density array
                      iz = 1
                      ! kg/m2        =      ppb  m    * ((mole tr/mole air)/ppb) * (kg tr/mole tr) *   (kg air/m3 air)  / (kg air/mole air)
                      pat(1:nx,1:ny) = pat(1:nx,1:ny) *       1e-9               *  specmolm(itr)  * dens(1:nx,1:ny,iz) /     xm_air
                      ! per second:
                      pat = pat / real(leo%dhour*3600)  ! kg/m2/s
                    !~ volume mixing ratio to molar concentration 
                    case ( 'ppb m -> umol m-2 s-1' )
                      ! index in density array
                      iz = 1
                      ! umol/m2        =      ppb  m    *   (kg air/m3 air)  / (kg air/mole air)/ (nmol/umol)  
                      pat(1:nx,1:ny)   = pat(1:nx,1:ny) * dens(1:nx,1:ny,iz) /     xm_air       / 1000  
                      ! per second:
                      pat = pat / real(leo%dhour*3600)  ! umol/m2/s
                    case ( 'ug/m3 m -> umol m-2 s-1' )
                      ! umol per hour   ug/m3 m          kg/mol        g/kg  
                      pat             =  pat        / specmolm(itr) / 1000 ! umol/m2/hr
                      ! per second
                      pat  =  pat / real(leo%dhour*3600)  
                    !~ unkown ...
                    case default
                ...
                ! Stomatal dry deposition budget dor ozone  
                case ( 'fstom_o3_hr' )
                  ! current hourly budget
                  pat = bud%o3flx_stomata_hour_lu(:,:,ilu)
                  
                  ! unit conversion (budget file is in nmol/m2)
                  select case ( 'nmol/m2 -> '//trim(LE_Dat(idat)%unit) )
                    ! other notations
                    case ( 'nmol/m2 -> umol m-2 s-1' )
                      ! umol/m2     =     nmol/m2  / nmol/umol
                      pat         =     - pat        / 1000  
                      ! per second:
                      pat = pat / real(leo%dhour*3600)  ! umol/m2/s
!!                     ug/m2       =     nmol/m2   * g/mol   * ug/g  * mol/nmol 
!                      pat         =     pat       * xm_o3  ! * 1e9  * 1e-9
                    case default
                      write (gol,'("do not know how to convert `",a,"` from `",a," m` to `",a,"`")') &
                              trim(specname(itr)), trim(specunit(itr)), trim(LE_Dat(idat)%unit); call goErr
                      TRACEBACK; status=1; return
                ...
                case ( 'area' )
                  ! point to meteo data:
                  call LE_Data_GetPointer( 'area', pdata, status, check_units=trim(LE_Dat(idat)%unit) )
                  IF_NOTOK_RETURN(status=1)
                  ! select:
  ...
  end subroutine LE_Output_vd_PutOut
