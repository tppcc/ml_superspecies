...
#ifdef with_m7 
    real, parameter   ::  partsize_NUCL   = 0.002e-6
    real, parameter   ::  partsize_AITKEN = 0.02e-6
    real, parameter   ::  partsize_ACCUM  = 0.2e-6
#endif 
      ...
      ispec = ispecs_aerosol(i_aerosol)
        ...
        select case ( specmode(ispec) )
          ...
          case ( NO_AEROSOL_MODE )
            ! for example H2SO4
          case ( AEROSOL_NUCL_MODE) 
            partsize(:,:,:,i_aerosol) = partsize_NUCL  ! m          
          case ( AEROSOL_AITKEN_MODE) 
            partsize(:,:,:,i_aerosol) = partsize_AITKEN  ! m
          case ( AEROSOL_ACCUM_MODE )
            partsize(:,:,:,i_aerosol) = partsize_ACCUM  ! m
#endif
