...
contains
  ...
  subroutine LE_Output_OMI_TRC_Label_Init( rcF, status )
    ...
    ! tracer:
    call rcF%Get( 'le.output.omi_trc_label.tracer', omi_trc%tracer_name, status )
    ...
    ! filename template:
    call rcF%Get( 'le.output.omi_trc_label.filenames', arch_filenames, status )
    IF_NOTOK_RETURN(status=1)
    
    ! top of tropo sphere to be used with kernel (Pa):
    call rcF%Get( 'le.output.omi_trc_label.ptropo', omi_trc_ptropo, status )
    IF_NOTOK_RETURN(status=1)
    
    call rcF%Get( 'le.output.omi_trc_label.top_hp', omi_trc%top_ph_entry, status )
    IF_NOTOK_RETURN(status=1)
    
    call rcF%Get( 'le.output.omi_trc_label.top_trc', omi_trc%top_trc_entry, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine CalcMeas_OMI_TRC( le_vmr_grid, le2omi_vcd_trop, status )
    ...
    if ( omi_trc%nmeas > 0 ) then
      ! storage:
      allocate( le2omi_vcd_trop(omi_trc%nmeas, 1:SA_nlabel), stat=status )
      IF_NOTOK_RETURN(status=1)
  ...
  subroutine leo_PutOut( leo, t, c, status, last )
    ...
    IF_NOTOK_RETURN(status=1)
    call dom%GatherV( omi_trc%le2omi_trc_vcd_trop, nmeas, le2omi_trc_vcd_trop_all, 2, status )
    IF_NOTOK_RETURN(status=1)
    ! analyzed values:
    ...
    IF_NOTOK_RETURN(status=1)
    call dom%GatherV( omi_trc%le2omi_trc_vcd_prof, nmeas, le2omi_trc_vcd_prof_all, (/1,3/), status )
    IF_NOTOK_RETURN(status=1)
    call dom%GatherV( omi_trc%le_trc_vcd_prof    , nmeas, le_trc_vcd_prof_all    , (/1,3/), status )
    IF_NOTOK_RETURN(status=1)
    call dom%GatherV( omi_trc%le_trc_vmr_prof    , nmeas, le_trc_vmr_prof_all    , (/1,3/), status )
    IF_NOTOK_RETURN(status=1)
