...
!     
!   call goSplitString( 'aa c', n, values, status [,sep=' '] )
!
...
!   
!   call goMatchValue( 'aa' , (/'aa ','bbb','c  ','ddd'/), ind, status [,quiet=.false.] )
!
...
!
!   call goMatchValues( 'aa c' , (/'aa ','bbb','c  ','ddd'/), n, values, inds, status [,quiet=.false.] )
!   call goMatchValues( '0 1 2', 1, 10                      , n, values      , status [,quiet=.false.] )
!
...
module GO_String
  ...
  interface goSplitString
    ...
    module procedure goSplitString_i
  end interface
  ...
  interface goCharToString
    module procedure goCharToString_1d
    module procedure goCharToString_2d
  end interface
 ...
 ! *****************************************************
  ...
  subroutine ReadFromLine_i( s, i, status, sep, default )
    ...
    if ( len_trim(s1) == 0 ) then
      ...
      if ( status /= 0 ) then
        write (gol,'("while reading integer out of `",a,"`")') trim(s); call goErr
        TRACEBACK; status=1; return
      end if
    end if
  ...
  subroutine goSplitString_i( line, n, values, status, sep )
  ...
  end subroutine goSplitString_i
  ...
  subroutine MatchValue_r( val, list, ind, status, quiet )
    ...
    if ( ind < 0 ) then
      if ( verbose ) then
        write (gol,'("name not supported:")'); call goErr
        write (gol,'("  value           : ",i6 )') val; call goErr
        write (gol,'("  possible values : ")'); call goErr
        do i = 1, nlist
          write (gol,'("    ",i4," ",i6)') i, list(i); call goErr
        end do
        TRACEBACK
      end if
      status=-1; return
  ...
  subroutine goVarValue_s( line, sep, var, is, val, status, verbose )
    ...
    if ( found ) then
      ...
      ! shout?
      if ( verb ) then
        write (gol,'("no value defined for `",a,"` in (sep `",a,"`, assign `",a,"`) : ")') &
                           trim(var), sep, is; call goErr
        write (gol,'("  ",a)') trim(line); call goErr
      end if
      ! warning status
      status = -1
  ...
  subroutine goVarValue_i( line, sep, var, is, val, status, verbose )
    ...
    if ( found ) then
      ...
      ! shout?
      if ( verb ) then
        write (gol,'("no value defined for `",a,"` in (sep `",a,"`, assign `",a,"`) : ",a)') &
                           trim(var), sep, is; call goErr
        write (gol,'("  ",a)') trim(line); call goErr
      end if
      ! warning status
  ...
  subroutine goVarValue_r( line, sep, var, is, val, status, verbose )
  ...
  subroutine goVarValue_l( line, sep, var, is, val, status, verbose )
  ...
  subroutine goCharToString_1d( chars, line, status )
    ...
    do i = 1, size(chars)
      ...
      ! check ..
      if ( i > len(line) ) then
        write (gol,'("found more characters than can be stored in line : ",i6)') len(line); call goErr
        TRACEBACK; status=1; return
      end if
      ! copy:
      line(i:i) = chars(i)
  ...
  end subroutine goCharToString_1d
  
  ! *
  
  subroutine goCharToString_2d( chars, lines, status )
    ...
    if ( size(chars,2) /= size(lines) ) then
      write (gol,'("shape of character array (",2i6,") differs from lines (",i6,")")') shape(chars), shape(lines); call goErr
      TRACEBACK; status=1; return
    end if
    ...
    do j = 1, size(lines)
      ! apply to line:
      call goCharToString_1d( chars(:,j), lines(j), status )
      IF_NOTOK_RETURN(status=1)
    end do
  ...
  end subroutine goCharToString_2d
  ...
  function goNtsTrim( s )
  
    ! --- in/out ---------------------------------
    
    character(len=*), intent(in)        ::  s
    character(len=len(s))               ::  goNtsTrim
