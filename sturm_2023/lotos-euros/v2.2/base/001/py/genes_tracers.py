...
class GeneSTracers( object ) :
    ...
    def __init__( self, txtfile, groups, props, aerrad, skip_tracers ) :
        ...
        Arguments:
          ...
          aerrad       : dictionairy with aerosol geometric radius/std.dev.  fields:
                           { 'sia_f' : ('0.0994','1.59'), ... }
        ...
        while True:
            ...
            for ifield in range(nfield) :
                ...
                if header == 'name' :
                    ...
                    # expect labels '<sia_f>' etc:
                    if field.startswith('<') and field.endswith('>') :
                        # extract label:
                        label = field.replace('<','').replace('>','')
                        # should be present ..
                        if label not in aerrad.keys() :
                            logger.error( 'label "%s" not in aerosol radius table:' % label )
                            for key in aerrad.keys() :
                                logger.error( '  %-10s : %s' % (key,aerrad[key]) )
                            #endfor
                            raise Exception
                        #endif
                        # reset to (mean,sigma) tupple, convert radius to m:
                        mu,sig = aerrad[label]
                        field = (mu+'e-6',sig)
                        #
                    elif len(field) == 0 :
                        ...
                        logger.error( 'unsupported field "%s" for aerosol radius' % field )
                        raise Exception
                    #endif
                    # store:
                    self.aerrad.append( field )                        
                    #
                elif header == 'groups' :
