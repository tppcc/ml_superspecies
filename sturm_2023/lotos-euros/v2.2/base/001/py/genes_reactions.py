            ...
            for prd in prds :            
                ...
                if (spec.lower() in labelled_specs) :
                    ...
                    for rct in rcts :
                        ...
                        # Check if reactant is labeled 
                        if rspec.lower() not in labelled_specs :

                            # If not --> check if reactant should have been labelled
                            if Carbon and not Nitrogen and specs.formula[irspec]['C'] > 0 :
                                logger.error( spec + ' is labelled, while origin ' + rspec + ' is not labelled')
                                logger.error( 'All tracers containing Carbon atom should be labelled' )
                                raise Exception
                            #endif
                            if Nitrogen and not Carbon and specs.formula[irspec]['N'] > 0 :
                                logger.error( spec + ' is labelled, while origin ' + rspec + ' is not labelled')
                                logger.error( 'All tracers containing Nitrogen atom should be labelled' )
                                raise Exception
                            #endif
                            if Sulphur and specs.formula[irspec]['S'] > 0 :
                                logger.error( spec + ' is labelled, while origin ' + rspec + ' is not labelled')
                                logger.error( 'All tracers containing Sulphur atom should be labelled' )
                                raise Exception
                            #endif
                            
                            continue
                        else :
                            # Increase number of reactants needed in labeling
                    ...
                    if nrspec_labelled == 0 :
                        ...
                        for rct in rcts :
                            ...
                            elif ( Nitrogen and not Carbon and  nnitrogen_rct == 0 ) :
                                # reactions PAR+OH result in loss of NO2 ( add with negative factor to production term)
                                # in labeling code, production per label will not match --> values will be normalized again
                                self.yp[ispec] = self.yp[ispec] + ' &\n       + (%1.2f * rk(ireac_%s)%s)' %(float(1.0/len(rcts)),label,rspecval_label) 
                            else :
                                continue
                            #endif
