...
contains
  ...
  subroutine Pollen_Data_Grass_Init( emg, rcF, rckey, t, status )
    ...
    if (polg_ver == 2 ) then 
      ...
      descriptionlist = (/'standard_name=StartCD', 'standard_name=lenCD','standard_name=Grass Percentage'/)
    else if ( polg_ver == 3 ) then
      allocate( descriptionlist(3) )
      descriptionlist = (/'var_name=start_calday_th_POLLEN_GRASS_m32', 'var_name=end_calday_th_POLLEN_GRASS_m32','var_name=emis_mask_POLLEN_GRASS_m32'/)
    end if
