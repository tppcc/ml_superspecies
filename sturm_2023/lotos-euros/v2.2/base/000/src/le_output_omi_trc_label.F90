...
!
! o call CalcMeas_OMI_TRC( le_vmr_grid, le_vmr_aloft, le2omi_vcd_trop, status )
...
!
! Output routines:
!   call Init( leo, rcfile, rckey, status )
...
module le_output_omi_trc_label
  ...
  public  ::  LE_Output_OMI_TRC_Label_Init, LE_Output_OMI_TRC_Label_Done
  public  ::  LE_Output_OMI_TRC_Label_GetMeas
  ...
  character(len=*), parameter   ::  mname = 'le_output_omi_trc_label'
  ...
  type T_LE_Output_OMI_TRC_Label
    ! name for this file:
  ...
  end type T_LE_Output_OMI_TRC_Label

  ! Storage to read the file with 1 day of measurements
  type OMI_TRC_label_type
    ! tracer name:
  ...
  type(OMI_TRC_label_type)  ::  omi_trc
...
contains
  ...
  subroutine LE_Output_OMI_TRC_Label_Init( rcfile, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_OMI_TRC_Label_Init'
    ...
    ! tracer:
    call ReadRc( rcF, 'le.output.omi_trc_label.tracer', omi_trc%tracer_name, status )
    IF_NOTOK_RETURN(status=1)
    ! search index:
    ...
    ! filename template:
    call ReadRc( rcF, 'le.output.omi_trc_label.filenames', arch_filenames, status )
    IF_NOTOK_RETURN(status=1)
    
    ! top of tropo sphere to be used with kernel (Pa):
    call ReadRc( rcF, 'le.output.omi_trc_label.ptropo', omi_trc_ptropo, status )
    IF_NOTOK_RETURN(status=1)
    
    call ReadRC( rcF, 'le.output.omi_trc_label.top_hp', omi_trc%top_ph_entry, status )
    IF_NOTOK_RETURN(status=1)
    
    call ReadRC( rcF, 'le.output.omi_trc_label.top_trc', omi_trc%top_trc_entry, status )
    IF_NOTOK_RETURN(status=1)
  ...
  end subroutine LE_Output_OMI_TRC_Label_Init
  ...
  subroutine LE_Output_OMI_TRC_Label_Done( status )
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_OMI_TRC_Label_Done'
  ...
  end subroutine LE_Output_OMI_TRC_Label_Done
  ...
  subroutine Alloc_OMI_TRC( omitrc, nrmeas, nrlev, status )
    ...
    type(OMI_TRC_Label_type), intent(inout) ::  omitrc
    ...
    if ( omitrc%nmeas == 0 ) then
       ...
       allocate( omitrc%le_trc_vmr_prof(1:nz,nrmeas,SA_nlabel)       )
       allocate( omitrc%le_trc_vcd_prof(1:nz,nrmeas,SA_nlabel)       )
       allocate( omitrc%le2omi_trc_vcd_prof(nrlev,nrmeas,SA_nlabel)    )
       allocate( omitrc%le2omi_trc_vcd_trop(nrmeas,SA_nlabel)          )
  ...
  subroutine LE_Output_OMI_TRC_Label_GetMeas( tint1, tint2, status )
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_OMI_TRC_Label_GetMeas'
    ...
    integer             ::  nrmeas, nrlev, imeas, i, nrmeas_in_domain
    ...
    if ( (tint1 < omi_trc_tr(1)) .or. (omi_trc_tr(2) < tint1) ) then
       ...
       do imeas = 1, nrmeas
          ...
          IF_NOTOK_RETURN(status=1)
          ! not in domain ?
          if ( .not. indomain ) omi_trc%status(imeas) = omi_trc%status(imeas) + status_outside
          if ( indomain )  then
       ...
       !write (gol,*) "    OMI/TRC: Nr of obs in domain = ", nrmeas_in_domain;     call goPr
  ...
  end subroutine LE_Output_OMI_TRC_Label_GetMeas
  ...
  subroutine CalcMeas_OMI_TRC( le_vmr_grid, le_vmr_aloft, le2omi_vcd_trop, status )
    ...
    use SA_Labeling, only : SA_frac, SA_nlabel, Labelled_Specs
    use SA_Labeling, only : Label_Aloft
    ...
    real, intent(out)         ::  le2omi_vcd_trop(maxomi,SA_nlabel) ! unit 1e15 molec/cm2
    integer, intent(out)      ::  status
    ...
    allocate( le_vmr(1:nz,1:SA_nlabel) )
    allocate( le_vcd(1:nz,1:SA_nlabel) )
    allocate( le2omi_vcd(1:nlevmax,1:SA_nlabel) )
    ...
    do imeas = 1, nmeasx
      ...
      le_hh(1:nz) = le_hh(0) + h_m(ix,iy,:)  ! m 

      ! model temperature:
      le_temp(1:nz) = temp(ix,iy,:)
      ...
      do SA_ilabel = 1, SA_nlabel
        le_vmr(1:nz, SA_ilabel) = le_vmr_grid (ix,iy,:) * SA_frac(ix,iy,:,Labelled_specs(omi_trc_itracer),SA_ilabel)      ! trc volume ppb
      end do
      ...
      do SA_ilabel = 1, SA_nlabel
        ...
        if ( SA_ilabel == Label_Aloft ) then
          call LE_Bound_Top_MergeProfiles( ix, iy, omi_trc_itracer, &
                                           omi_trc%top_ph_entry, omi_trc%top_trc_entry, &
                                           le_ph(0:nz), le_vcd(1:nz,SA_ilabel)*du_to_kgm2, 'kg/m2', &
                                           mer_phlev, mer_vcd, status, label_aloft=.true. )
          IF_NOTOK_RETURN(status=1)
        else 
          call LE_Bound_Top_MergeProfiles( ix, iy, omi_trc_itracer, &
                                           omi_trc%top_ph_entry, omi_trc%top_trc_entry, &
                                           le_ph(0:nz), le_vcd(1:nz,SA_ilabel)*du_to_kgm2, 'kg/m2', &
                                           mer_phlev, mer_vcd, status, label_aloft=.false. )
          IF_NOTOK_RETURN(status=1)
        end if       
        ...
        do ilev = 1, nlevx
          ...
          if ( min( omi_ph(ilev-1), omi_ph(ilev) ) >= omi_trc_ptropo ) then
            ...
            call IntervalSum( -1.0*mer_phlev, mer_vcd, &
                               ...
                               le2omi_vcd(ilev,SA_ilabel), ilast, status )
            if ( status /= 0 ) then
        ...
        le2omi_vcd_trop(imeas,SA_ilabel) = 0.0
        do ilev = 1, nlevx
           le2omi_vcd_trop(imeas,SA_ilabel) = le2omi_vcd_trop(imeas,SA_ilabel) + &
                omi_trc%kernel(ilev,imeas) * le2omi_vcd(ilev,SA_ilabel)
        end do
  ...
  subroutine leo_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    type(T_LE_Output_OMI_TRC_Label), intent(out)  ::  leo
    character(len=*), intent(in)            ::  rcfile
  ...
  subroutine leo_Done( leo, status )
    ...
    type(T_LE_Output_OMI_TRC_Label), intent(inout)      ::  leo
    integer, intent(out)                      ::  status
  ...
  subroutine leo_PutOut( leo, t, c, status, last )
    ...
    if ( omi_trc%nmeas < 1 .or. omi_trc%nmeas_intimestep < 1 ) then
       ! info ...
       call wrtgol('    OMI/TRC: put out return: no TRC obs for time : ',t); call goPr
       status=0; return
    end if

    ! storage:
    allocate( le2omi_trc_vcd_trop(1:maxomi, SA_nlabel) )

    ! simulate omi stuff:
    call CalcMeas_OMI_TRC( c(:,:,:,omi_trc_itracer), caloft(:,:,omi_trc_itracer), le2omi_trc_vcd_trop, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! storage:
    deallocate( le2omi_trc_vcd_trop )
    ...
    ! filter:
    allocate( ii(1:omi_trc%nmeas) )
    nmeas = 0
    do i = 1, omi_trc%nmeas
      ! skip pixels outside domain:
      if ( iand(omi_trc%status(i),status_outside) /= 0 ) cycle
      ! skip pixels without data:
      if ( iand(omi_trc%status(i),status_nodata) /= 0 ) cycle
      ! add index:
      nmeas = nmeas + 1
      ii(nmeas) = i
    end do
    write (gol,'("    OMI/TRC: pixels in domain and with data : ",i6," / ",i6)') nmeas, omi_trc%nmeas; call goPr
    ! empty ?
    if ( nmeas < 1 ) then
      write (gol,'("    OMI/TRC: no pixels that survived the filter ...")'); call goPr
      deallocate( ii )
      status=0; return
    end if
    ...
    leo%opened = .true.
     ...
     print *, 'meas-omi name created ', trim(leo%fname)
    ...
    ! define dimensions:
    status = NF90_Def_Dim( leo%ncid, 'omi_obs', nmeas, leo%dimid_omi_obs )
    IF_NF90_NOTOK_RETURN(status=1)
    ...
    do ient = 1, nent
      ...
      select case ( ient )
        ...
        case ( ient_le2omi_vcd_trop )
          ...
          status = NF90_Def_Var( leo%ncid, trim(varname), NF90_REAL, (/leo%dimid_omi_obs,leo%dimid_label/), varid )
          IF_NF90_NOTOK_RETURN(status=1)
        ...
        case ( ient_le2omi_vcd )
          ...
          status = NF90_Def_Var( leo%ncid, trim(varname), NF90_REAL, (/leo%dimid_omi_lev,leo%dimid_omi_obs,leo%dimid_label/), varid )
          IF_NF90_NOTOK_RETURN(status=1)
        ...
        case ( ient_le_vcd )
          ...
          status = NF90_Def_Var( leo%ncid, trim(varname), NF90_REAL, (/leo%dimid_le_lev,leo%dimid_omi_obs,leo%dimid_label/), varid )
          IF_NF90_NOTOK_RETURN(status=1)
    ...
    status = NF90_Put_Var( leo%ncid, leo%varid_lon, omi_trc%longitude(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_lat, omi_trc%latitude(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_ilon, omi_trc%ix(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_ilat, omi_trc%iy(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    ! date in year,month,etc
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_date, omi_trc%date(:,ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    ...
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_time, omi_trc%time(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_pressure_levels, omi_trc%pressure_levels(:,ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_kernel         , omi_trc%kernel(:,ii(1:nmeas))          )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_cloud_fraction , omi_trc%cloud_fraction(ii(1:nmeas))  )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_y, omi_trc%vcd_trop(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    
    status = NF90_Put_Var( leo%ncid, leo%varid_r, omi_trc%sigma_vcd_trop(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)

    status = NF90_Put_Var( leo%ncid, leo%varid_omi_status, omi_trc%status(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    ...
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_analysed, dummy(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    deallocate ( dummy )
    ...
    status = NF90_Put_Var( leo%ncid, leo%varid_omi_screened, dummy(ii(1:nmeas)) )
    IF_NF90_NOTOK_RETURN(status=1)
    deallocate ( dummy )
    ...
    ! write:
    status = NF90_Put_Var( leo%ncid, leo%varid_le_altitude_levels, &
         omi_trc%le_altitude_levels(:,ii(1:nmeas)), &
         start=(/1,1/), count=(/nz+1,nmeas/) )
    IF_NF90_NOTOK_RETURN(status=1)
    status = NF90_Put_Var( leo%ncid, leo%varid_le_pressure_levels, &
         omi_trc%le_pressure_levels(:,ii(1:nmeas)), &
         start=(/1,1/), count=(/nz+1,nmeas/) )
    IF_NF90_NOTOK_RETURN(status=1)

    ! write:
    status = NF90_Put_Var( leo%ncid, leo%varid(ient_le2omi_vcd_trop), &
         omi_trc%le2omi_trc_vcd_trop(ii(1:nmeas),:), &
         start=(/1,1/), count=(/nmeas,SA_nlabel/) )
    IF_NF90_NOTOK_RETURN(status=1)
    status = NF90_Put_Var( leo%ncid, leo%varid(ient_le2omi_vcd), &
         omi_trc%le2omi_trc_vcd_prof(:,ii(1:nmeas),:), &
         start=(/1,1/), count=(/nlev,nmeas,SA_nlabel/) )
    IF_NF90_NOTOK_RETURN(status=1)
    status = NF90_Put_Var( leo%ncid, leo%varid(ient_le_vcd), &
         omi_trc%le_trc_vcd_prof(:,ii(1:nmeas),:), &
         start=(/1,1/), count=(/nz,nmeas,SA_nlabel/) )
    IF_NF90_NOTOK_RETURN(status=1)
    status = NF90_Put_Var( leo%ncid, leo%varid(ient_le_vmr), &
         omi_trc%le_trc_vmr_prof(:,ii(1:nmeas),:), &
         start=(/1,1/), count=(/nz,nmeas,SA_nlabel/) )
    IF_NF90_NOTOK_RETURN(status=1)
...
end module le_output_omi_trc_label
