...
module LE_Output
  ...
  use LE_Output_aod    , only : T_LE_Output_aod
  ...
  type T_LE_Output
    ...
    ! aod output
    integer                               ::  naod
    character(len=16)                     ::  file_aod(maxfile)
    ! omi trc output
    integer                               ::  nomi_trc
  ...
  type T_LE_OutputState
    ...
    ! aod output
    type(T_LE_Output_aod), pointer        ::  output_aod(:)
    ! omi trc output
...
contains
  ...
  subroutine leo_Init( leo, rcF, rckey, status )
    ...
    ! store:
    leo%rcfile = trim(rcF%fname)
    leo%rckey  = trim(rckey)

    ! lockfile to be written ?
    call ReadRc( rcF, trim(leo%rckey)//'.lockfile.apply', leo%lockfile_apply, status )
    IF_NOTOK_RETURN(status=1)
    if ( leo%lockfile_apply ) then
       call ReadRc( rcF, trim(leo%rckey)//'.lockfile.dhour', leo%lockfile_dhour, status )
       IF_NOTOK_RETURN(status=1)
       call ReadRc( rcF, trim(leo%rckey)//'.lockfile.template', leo%lockfile_template, status )
       IF_NOTOK_RETURN(status=1)
    end if

    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.dat.files', list, status )
    IF_NOTOK_RETURN(status=1)
      ...
      if ( leo%ndat > 0 ) then
        ...
        do i = 1, leo%ndat
          call LE_Output_Dat_Init( leo%output_dat(i), trim(leo%rcfile), &
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.dat2.files', list, status )
    IF_NOTOK_RETURN(status=1)
      ...
      if ( leo%ndat2 > 0 ) then
        ...
        ! loop over files to be written:
        do i = 1, leo%ndat2
          call LE_Output_Dat2_Init( leo%output_dat2(i), trim(rcF%fname), &
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.conc.files', list, status )
    IF_NOTOK_RETURN(status=1)
...
#ifdef with_labeling
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.label.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.labelled_conc.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files
    call ReadRc( rcF, trim(leo%rckey)//'.emis_label.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.omi_trc_label.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files
    call ReadRc( rcF, trim(leo%rckey)//'.drydep_label.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files
    call ReadRc( rcF, trim(leo%rckey)//'.wetdep_label.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files
    call ReadRc( rcF, trim(leo%rckey)//'.budget_label.files', list, status )
    IF_NOTOK_RETURN(status=1)
...
#endif
!    ! read files:
!    call ReadRc( rcF, trim(leo%rckey)//'.dep.files', list, status )
!    IF_NOTOK_RETURN(status=1)
!
!      ! extract parts:
...
!      end if
...
#ifdef with_radiation
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.rad.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.emis.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.vd.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.vd-diag.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.drydep.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.wetdep.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.budget.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.column.files', list, status )
    IF_NOTOK_RETURN(status=1)
    ...
    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.aod.files', list, status )
    IF_NOTOK_RETURN(status=1)

      ! extract parts:
      call goSplitString( list, leo%naod, leo%file_aod, status )
      IF_NOTOK_RETURN(status=1)

    ! read files:
    call ReadRc( rcF, trim(leo%rckey)//'.omi_trc.files', list, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine leo_Setup( leo, t1, t2, status )

    use GO               , only : TDate, Midnight, IncrDate, operator(+)
  ...
  subroutine leo_PutOut( leo, t, status )
    ...
    if ( leo%ndat > 0 ) then
      do i = 1, leo%ndat
        call LE_Output_Dat_PutOut( leo%output_dat(i), t, status )
        IF_NOTOK_RETURN(status=1)
      end do
    end if
    ...
    if ( leo%ndat2 > 0 ) then
      do i = 1, leo%ndat2
        call LE_Output_Dat2_PutOut( leo%output_dat2(i), t, status )
        IF_NOTOK_RETURN(status=1)
      end do
    end if
  ...
  subroutine leos_Init( leos, leo, name, status )
    ...
    use LE_Output_AOD    , only : LE_Output_AOD_Init
    ...
    if ( leo%nconc > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nconc
        call LE_Output_Conc_Init( leos%output_conc(i), trim(leo%rcfile), &
    ...
    if ( leo%nlabel > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nlabel
        call LE_Output_label_Init( leos%output_label(i), trim(leo%rcfile), &
    ...
    if ( leo%nlabelled_conc > 0 ) then
      ...
      do i = 1, leo%nlabelled_conc
        call LE_Output_labelled_Conc_Init( leos%output_labelled_conc(i), trim(leo%rcfile), &
    ...
    if ( leo%nemislabel > 0  ) then
      ...
      do i = 1, leo%nemislabel
        call LE_Output_Emis_label_Init( leos%output_emis_label(i), trim(leo%rcfile), &
    ...
    if ( leo%nomi_trc_label > 0 ) then
      ...
      call LE_Output_OMI_TRC_Label_Init( trim(leo%rcfile), status )
      IF_NOTOK_RETURN(status=1)
      ! storage for output stuff:
      allocate( leos%output_omi_trc_label(leo%nomi_trc_label) )
      ! loop over files to be written:
      do i = 1, leo%nomi_trc_label
        call Init( leos%output_omi_trc_label(i), trim(leo%rcfile), &
    ...
    if ( leo%ndrydeplabel > 0 ) then
      ...
      do i = 1, leo%ndrydeplabel
        call LE_Output_Drydep_label_Init( leos%output_drydep_label(i), trim(leo%rcfile), &
    ...
    if ( leo%nwetdeplabel > 0 ) then
      ...
      do i = 1, leo%nwetdeplabel
        call LE_Output_Wetdep_label_Init( leos%output_wetdep_label(i), trim(leo%rcfile), &
    ...
    if ( leo%nbudgetlabel > 0 ) then
      ...
      do i = 1, leo%nbudgetlabel
        call LE_Output_Budget_label_Init( leos%output_budget_label(i), trim(leo%rcfile), &
    ...
    if ( leo%nrad > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nrad
        call LE_Output_Rad_Init( leos%output_rad(i), trim(leo%rcfile), &
    ...
    if ( leo%nemis > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nemis
        call LE_Output_Emis_Init( leos%output_emis(i), trim(leo%rcfile), &
    ...
    if ( leo%nvd > 0 ) then
       ...
       ! loop over files to be written:
       do i = 1, leo%nvd
          call LE_Output_vd_Init( leos%output_vd(i), trim(leo%rcfile), &
    ...
    if ( leo%nvd_diag > 0 ) then
       ...
       ! loop over files to be written:
       do i = 1, leo%nvd_diag
          call LE_Output_vd_diag_Init( leos%output_vd_diag(i), trim(leo%rcfile), &
    ...
    if ( leo%ndrydep > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%ndrydep
        call LE_Output_DryDep_Init( leos%output_drydep(i), trim(leo%rcfile), &
    ...
    if ( leo%nwetdep > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nwetdep
        call LE_Output_WetDep_Init( leos%output_wetdep(i), trim(leo%rcfile), &
    ...
    if ( leo%nbudget > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%nbudget
        call LE_Output_budget_Init( leos%output_budget(i), trim(leo%rcfile), &
    ...
    if ( leo%ncolumn > 0 ) then
      ...
      ! loop over files to be written:
      do i = 1, leo%ncolumn
        call LE_Output_Column_Init( leos%output_column(i), trim(leo%rcfile), &
    ...
    if ( leo%naod > 0 ) then
      ! storage for output stuff:
      allocate( leos%output_aod(leo%naod) )
      ! loop over files to be written:
      do i = 1, leo%naod
        call LE_Output_AOD_Init( leos%output_aod(i), trim(leo%rcfile), &
                     trim(leo%rckey), 'aod', leo%file_aod(i), &
                     trim(leos%name), status )
        IF_NOTOK_RETURN(status=1)
      end do
    end if
    ...
    if ( leo%nomi_trc > 0 ) then
      ! setup OMI TRC module:
      call LE_Output_OMI_TRC_Init( trim(leo%rcfile), status )
      IF_NOTOK_RETURN(status=1)
      ! storage for output stuff:
      allocate( leos%output_omi_trc(leo%nomi_trc) )
      ! loop over files to be written:
      do i = 1, leo%nomi_trc
        call Init( leos%output_omi_trc(i), trim(leo%rcfile), &
  ...
  subroutine leos_Done( leos, leo, status )
    ...
    use LE_Output_AOD    , only : LE_Output_AOD_Done
    ...
    if ( leo%naod > 0 ) then
      ...
      do i = 1, leo%naod
        ! done with output file:
        call LE_Output_AOD_Done( leos%output_aod(i), status )
        IF_NOTOK_RETURN(status=1)
      end do
      ! clear:
      deallocate( leos%output_aod )
    end if
  ...
  subroutine leos_PutOut( leos, leo, t, c, cg, bud, status, last, without_data )
    ...
    use LE_Output_AOD    , only : LE_Output_AOD_PutOut
    ...
    if ( leo%naod > 0 ) then
      ...
      ! loop over aod files to be written:
      do i = 1, leo%naod
        ! write instantenous fields:
        call LE_Output_AOD_PutOut( leos%output_aod(i), t, c, status )
        IF_NOTOK_RETURN(status=1)
      end do
