...
module LE_Data_Common
  ...
  ! number of levels:
  integer                       ::  nlev
  ! settings for mixlayer definition:
  ...
  type(T_Levs_Hyb)              ::  hyb_top    ! (1:nlev+1)
...
contains
  ...
  subroutine LE_Data_Common_Init( rcF, status )
    ...
    select case ( trim(levels_type) )
      ...
      case ( 'hyblevel' )
        ! read settings:
        call LE_Data_Common_HybLevel_Init( rcF, status )
        IF_NOTOK_RETURN(status=1)
      !~
      case ( 'metlevel' )
        ! read settings:
  ...
  subroutine LE_Data_Common_HybLevel_Init( rcF, status )
    ...
    ! storage:
    allocate( hyai(0:nlev+1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( hybi(0:nlev+1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( hyam(nlev+1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( hybm(nlev+1), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( filled(0:nlev+1), stat=status )
    IF_NOTOK_RETURN(status=1)
    ...
    if ( .not. all(filled) ) then
      write (gol,'("not all ",i0," half levels (",i0," incl top) filled:")') nlev, nlev+1; call goErr
    ...
    ! mid levels:
    hyam = 0.5 * ( hyai(0:nlev-1) + hyai(1:nlev) )
    hybm = 0.5 * ( hybi(0:nlev-1) + hybi(1:nlev) )

    ! define levels:
    call hyb%Init( hyam(1:nlev), hybm(1:nlev), hyai(0:nlev), hybi(0:nlev), status )
    IF_NOTOK_RETURN(status=1)
    ! idem incl top layer:
    call hyb_top%Init( hyam(1:nlev+1), hybm(1:nlev+1), hyai(0:nlev+1), hybi(0:nlev+1), status )
    IF_NOTOK_RETURN(status=1)
