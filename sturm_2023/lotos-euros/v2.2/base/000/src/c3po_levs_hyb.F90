...
!
! NOTE: discussion on "formula_terms" and "bounds":
!   http://cf-trac.llnl.gov/trac/ticket/147
!
!######################################################################
!
...
module C3PO_Levs_Hyb
  ...
  contains
    ...
    !
    procedure   ::                  Levs_Hyb_Ps_to_P_1d
    procedure   ::                  Levs_Hyb_Ps_to_P_2d
    generic     ::  Ps_to_P      => Levs_Hyb_Ps_to_P_1d, &
                                    Levs_Hyb_Ps_to_P_2d
    !
    procedure   ::                  Levs_Hyb_Ps_to_Ph_1d
    procedure   ::                  Levs_Hyb_Ps_to_Ph_2d
    generic     ::  Ps_to_Ph     => Levs_Hyb_Ps_to_Ph_1d, &
                                    Levs_Hyb_Ps_to_Ph_2d
    !
    procedure   ::                  Levs_Hyb_Ps_to_dP_2d
    procedure   ::                  Levs_Hyb_Ps_to_dP_3d
    generic     ::  Ps_to_dP     => Levs_Hyb_Ps_to_dP_2d, &
                                    Levs_Hyb_Ps_to_dP_3d
  end type T_Levs_Hyb
...
contains
  ...
  subroutine Levs_Hyb_Init_ab_i( self, ap, b, ap_i, b_i, status  )
    ...
    class(T_Levs_Hyb), intent(out)  ::  self
  ...
  subroutine Levs_Hyb_Init_coarsen( self, hyb, nn, status, mapping  )
    ...
    class(T_Levs_Hyb), intent(out)  ::  self
    class(T_Levs_Hyb), intent(in)   ::  hyb
  ...
  subroutine Levs_Hyb_Done( self, status )
    ...
    class(T_Levs_Hyb), intent(inout)    ::  self
    integer, intent(out)                ::  status
  ...
  subroutine Levs_Hyb_Ps_to_P_1d( self, ps, p, status )
  
    ! --- in/out ---------------------------------
    
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:)    ! (np) surface pressure [Pa]
    real, intent(out)                   ::  p(:,:)   ! (np,nlev) full.lev.pres. [Pa]
    integer, intent(out)                ::  status

    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Levs_Hyb_Ps_to_P_1d'
    ...
    ! check ..
    if ( any( shape(p) /= (/size(ps),self%nlev/) ) ) then
      write (gol,'("unexpected shape of p : ",2i6)') shape(p); call goErr
      write (gol,'("  grid size ps  : ",i6)') size(ps); call goErr
    ...
    do i = 1, size(ps)
      ...
      p(i,:) = self%ap + self%b * ps(i)  ! [Pa]
    end do
    
    ! ok
    status = 0
  
  end subroutine Levs_Hyb_Ps_to_P_1d
  
  
  ! *
  
  
  subroutine Levs_Hyb_Ps_to_P_2d( self, ps, p, status )
  
    ! --- in/out ---------------------------------
    
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:,:)    ! (nlon,nlat) surface pressure [Pa]
    real, intent(out)                   ::  p(:,:,:)   ! (nlon,nlat,nlev) full.lev.pres. [Pa]
    integer, intent(out)                ::  status

    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Levs_Hyb_Ps_to_P_2d'
    ...
    ! check ..
    if ( any( shape(p) /= (/size(ps,1),size(ps,2),self%nlev/) ) ) then
      write (gol,'("unexpected shape of p : ",3i6)') shape(p); call goErr
      write (gol,'("  grid size ps  : ",2i6)') shape(ps); call goErr
      write (gol,'("  levels        : ",i6)') self%nlev; call goErr
      TRACEBACK; status=1; return
    end if
    ...
    do j = 1, size(ps,2)
      do i = 1, size(ps,1)
        ! apply interface coeffs:
        p(i,j,:) = self%ap + self%b * ps(i,j)  ! [Pa]
      end do
    end do
  ...
  end subroutine Levs_Hyb_Ps_to_P_2d
  ...
  subroutine Levs_Hyb_Ps_to_Ph_1d( self, ps, ph, status )
    ...
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:)    ! (np) surface pressure [Pa]
    real, intent(out)                   ::  ph(:,:)  ! (np,0:nlev) half.lev.pres. [Pa]
    integer, intent(out)                ::  status
  ...
  subroutine Levs_Hyb_Ps_to_Ph_2d( self, ps, ph, status )
  
    ! --- in/out ---------------------------------
    
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:,:  ) ! (nx,ny       ) surface pressure [Pa]
    real, intent(out)                   ::  ph(:,:,:) ! (nx,ny,0:nlev) half.lev.pres. [Pa]
    integer, intent(out)                ::  status

    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Levs_Hyb_Ps_to_Ph_2d'
    ...
    ! check ..
    if ( any( shape(ph) /= (/size(ps,1),size(ps,2),self%nlev+1/) ) ) then
      write (gol,'("unexpected shape of ph : ",3i6)') shape(ph); call goErr
      write (gol,'("  grid size ps  : ",2i6)') shape(ps); call goErr
      write (gol,'("  half levels   : ",i6," + 1")') self%nlev; call goErr
      TRACEBACK; status=1; return
    end if
    ...
    do j = 1, size(ps,2)
      do i = 1, size(ps,1)
        ! apply interface coeffs:
        ph(i,j,:) = self%ap_i + self%b_i * ps(i,j)  ! [Pa]
      end do
    end do
  ...
  end subroutine Levs_Hyb_Ps_to_Ph_2d
  
  
  ! ***
  
  
  subroutine Levs_Hyb_Ps_to_dP_2d( self, ps, dp, status )
  
    ! --- in/out ---------------------------------
    
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:  ,:) ! (nx     ,nt) surface pressure [Pa]
    real, intent(out)                   ::  dp(:,:,:) ! (nx,nlev,nt) delta pressure [Pa]
    integer, intent(out)                ::  status

    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Levs_Hyb_Ps_to_dP_2d'
  ...
  end subroutine Levs_Hyb_Ps_to_dP_2d
  
  
  ! *
  
  
  subroutine Levs_Hyb_Ps_to_dP_3d( self, ps, dp, status )
  
    ! --- in/out ---------------------------------
    
    class(T_Levs_Hyb), intent(in)       ::  self
    real, intent(in)                    ::  ps(:,:  ,:) ! (nx,ny     ,nt) surface pressure [Pa]
    real, intent(out)                   ::  dp(:,:,:,:) ! (nx,ny,nlev,nt) delta pressure [Pa]
    integer, intent(out)                ::  status

    ! --- const --------------------------------------

    character(len=*), parameter  ::  rname = mname//'/Levs_Hyb_Ps_to_dP_3d'
  ...
  end subroutine Levs_Hyb_Ps_to_dP_3d
