...
module LE_Bound_Common
  ...
  real, allocatable     ::  bc_west (:,:,:)
  real, allocatable     ::  bc_east (:,:,:)
  real, allocatable     ::  bc_north(:,:,:)
  real, allocatable     ::  bc_south(:,:,:)
  real, allocatable     ::  caloft  (:,:,:)
...
contains
  ...
  subroutine LE_Bound_Common_Init( status )
    ...
    ! storage:
    allocate( bc_west(ny,nz,nspec), stat=status )
    IF_NOTOK_RETURN(status=1)
    allocate( bc_east(ny,nz,nspec), stat=status ) 
    IF_NOTOK_RETURN(status=1)
    allocate( bc_north(nx,nz,nspec), stat=status ) 
    IF_NOTOK_RETURN(status=1)
    allocate( bc_south(nx,nz,nspec), stat=status ) 
    IF_NOTOK_RETURN(status=1)
    allocate( caloft(nx,ny,nspec), stat=status ) 
    IF_NOTOK_RETURN(status=1)
