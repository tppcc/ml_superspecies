...
contains
  ...
  subroutine Pollen_Data_Olive_Init( emo, rcF, rckey, t, status )
    ...
    descriptionlist = (/'standard_name=Olive Percentage', 'standard_name=Heatsum_thresh'/)
    ...
    allocate( emo%heatsum_diff(ugg%nlon,ugg%nlat) )
    emo%heatsum_diff = heatsum_diff_olives

    allocate( emo%heatsum_end(ugg%nlon,ugg%nlat) )
    emo%heatsum_end = emo%heatsum_start + emo%heatsum_end

    allocate( emo%heatsum_grow(ugg%nlon,ugg%nlat) )
    emo%heatsum_grow = 0.0

    ! fill in intensity array with only values 1        
    allocate( emo%intensity(ugg%nlon,ugg%nlat)  )
    emo%intensity = 1.0

       
    ! initialize emissions
    allocate(emo%emis(ugg%nlon,ugg%nlat) )
    emo%emis = 0.0   
    
    allocate(emo%ripened_left(ugg%nlon,ugg%nlat) )
    ...
    ! initialize heatsum
    allocate(emo%heatsum(ugg%nlon,ugg%nlat) )    
