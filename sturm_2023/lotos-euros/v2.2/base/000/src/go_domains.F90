...
!  ! process topology (domain connection)
!  type T_DomainTopo
!    ! number of dimensions:
!    integer                ::  ndim
!    ! topology shape:
!    integer, allocatable   ::  shp(:)  ! (ndim)
!    ! process id per domain, assume max 2 dims:
!    integer, allocatable   ::  pid(:,:)  ! (shp(1),shp(2))
!    ! topology tile indices per processor:
!    integer, allocatable   ::  loc(:,:)  ! (ndim,0:nproc-1)
!    !
!  contains
!    procedure   ::  Init         => DomainTopo_Init
!    procedure   ::  Done         => DomainTopo_Done
!    procedure   ::  Show         => DomainTopo_Show
!  end type T_DomainTopo
...
!    procedure   ::  Show            => Domains_Show
    ...
    !
!    procedure   ::  ExchangeHaloEdgeX  => Domains_ExchangeHaloEdgeX_3d_r
!    procedure   ::  ExchangeHaloEdgeY  => Domains_ExchangeHaloEdgeY_3d_r
    ...
    generic     ::  GatherV            => Domains_GatherV_2d_r, &
...
!  
!  !
!  ! initialize domain topology, 
!  ! assign number processors
!  !
!
!  subroutine DomainTopo_Init( self, shp, status )
!
!    use GO_Comm , only : goc
!
!    ! --- const --------------------------------------
!
!    character(len=*), parameter  ::  rname = mname//'/DomainTopo_Init'
!    
!    ! --- in/out ---------------------------------
!    
!    class(T_DomainTopo), intent(out)  ::  self
...
!    
!    ! check ..
!    if ( product(shp) /= goc%npes ) then
!      write (gol,'("number of processes (",i0,") should be the same as number of domains (",i0,")")') &
!               goc%npes, product(shp); call goErr
!      write (gol,*) 'domain topology: ', shp; call goErr
...
!    allocate( self%loc(self%ndim,0:goc%npes), stat=status )
!    IF_NOT_OK_RETURN(status=1)
!
!    ! store:
!    self%shp = shp    
...
!    if ( self%ndim == 1 ) then
!      ! storage:
!      allocate( self%pid(self%shp(1),1), stat=status )
!      IF_NOT_OK_RETURN(status=1)
!      ! processor id:
!      p = 0
!      ! loop over domains:
...
!      ! storage:
!      allocate( self%pid(self%shp(1),self%shp(2)), stat=status )
!      IF_NOT_OK_RETURN(status=1)
!      ! processor id:
!      p = 0
!      ! loop over domains:
...
!    status = 0
!
!  end subroutine DomainTopo_Init
!
!
!  ! ***
!  
!  
!  subroutine DomainTopo_Done( self, status )
!    
!    ! --- in/out ---------------------------------
!    
!    class(T_DomainTopo), intent(inout)    ::  self
!    integer, intent(out)                  ::  status
!
!    ! --- const --------------------------------------
!
!    character(len=*), parameter  ::  rname = mname//'/DomainTopo_Done'
!    
!    ! --- local ----------------------------------
!    
!    ! --- begin ----------------------------------
...
!    IF_NOT_OK_RETURN(status=1)
!    deallocate( self%loc, stat=status )
!    IF_NOT_OK_RETURN(status=1)
!    deallocate( self%pid, stat=status )
!    IF_NOT_OK_RETURN(status=1)
!    
!    ! ok
!    status = 0
!
!  end subroutine DomainTopo_Done
!
!
!  ! ***
!  
!  
!  subroutine DomainTopo_Show( self, status )
!
!    use GO_Comm , only : goc
!    
!    ! --- in/out ---------------------------------
!    
!    class(T_DomainTopo), intent(in)       ::  self
!    integer, intent(out)                  ::  status
!
!    ! --- const --------------------------------------
!
!    character(len=*), parameter  ::  rname = mname//'/DomainTopo_Show'
!    
!    ! --- local ----------------------------------
!    
!    integer       ::  p
...
!    
!    ! info ...
!    write (gol,'("domain topology:")'); call goPr
!    do p = 0, goc%npes-1
!      if ( self%ndim == 1 ) then
!        write (gol,'("  pe ",i2," domain (",i2,")")') p, self%loc(:,p); call goPr
!      else if ( self%ndim == 2 ) then
!        write (gol,'("  pe ",i2," domain (",i2,",",i2,")")') p, self%loc(:,p); call goPr
!      else
!        write (gol,'("unsupported number of dimensions ",i0)') self%ndim; call goErr
!        TRACEBACK; status=1; return
!      end if
...
!    status = 0
!
!  end subroutine DomainTopo_Show
...
!  !
!  ! initialize local domain using its global bounds 
!  ! (provided by each local processor)
!  !
!
...
!  
!  !
!  ! initialize subdomains as 1D array;
...
!    ! --- const --------------------------------------
!
!    character(len=*), parameter  ::  rname = mname//'/Domains_InitLocal'
!    
!    ! --- in/out ---------------------------------
!    
!    class(T_Domains), intent(out)     ::  self
...
!    ! --- begin ----------------------------------
!    
!    ! topology is 1D over all processes:
!    call self%topo%Init( (/goc%npes/), status )
!    IF_NOT_OK_RETURN(status=1)
!    
!    ! storage:
!    allocate( counts(0:goc%npes-1), stat=status )
!    IF_NOT_OK_RETURN(status=1)
...
!    call self%Init( (/displs(goc%id)+1/), (/displs(goc%id)+counts(goc%id)/), status )
!    IF_NOT_OK_RETURN(status=1)
!
!    ! storage:
!    deallocate( counts, stat=status )
!    IF_NOT_OK_RETURN(status=1)
...
!    ! initialize topology:
!    call self%topo%Init( cart_dims, status )
!    IF_NOT_OK_RETURN(status=1)
...
#else
    ! check ...
    if ( product(cart_dims) /= 1 ) then
      write (gol,'("decomposition on more than one domain (",i0,"), but MPI interface not enabled ...")') &
                         product(cart_dims); call goErr
      TRACEBACK; status=1; return
    end if
    ! dummy:
    self%cart_coords = 0
...
!  subroutine Domains_Show( self, status )
!
!    use GO_Comm, only : goc
!    
!    ! --- in/out ---------------------------------
...
!    ! --- const --------------------------------------
!
!    character(len=*), parameter  ::  rname = mname//'/Domains_Show'
!    
!    ! --- local ----------------------------------
!    
!    integer       ::  p
...
!      write (gol,'("  offset: ",i0,",",i0)') self%off(:,goc%id); call goPr
!      write (gol,'("  shape : ",i0,",",i0)') self%shp(:,goc%id); call goPr
!      write (gol,'("  size  : ",i0,",",i0)') self%n(goc%id); call goPr
!    else
!      write (gol,'("unsupported number of dimensions ",i0)') self%ndim; call goErr
!      TRACEBACK; status=1; return
!    end if
