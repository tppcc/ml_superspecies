...
contains
  ...
  subroutine LE_Output_Labelled_Conc_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    ! tracer names :
    call ReadRc( rcF, trim(basekey)//'.specs', output_tracer_names, status )
    IF_NOTOK_RETURN(status=1)

    ! leveldescriptions
    call ReadRc( rcF, trim(basekey)//'.levels', output_level_numbers, status )
    IF_NOTOK_RETURN(status=1)       
  ...
  subroutine LE_Output_Labelled_Conc_PutOut( leo, t, c, cg, status )
    ...
    use NetCDF , only : NF90_NOCLOBBER, NF90_CLOBBER, NF90_UNLIMITED
