...
contains
  ...
  subroutine LE_Output_Dat2_Init( leo, rcfile, rckey, typ, name, status )
    ...
    ! grid type:
    call ReadRc( rcF, trim(basekey)//'.gridtype', leo%gridtype, status )
    IF_NOTOK_RETURN(status=1)
...
!    end select
  ...
  subroutine LE_Output_Dat2_Done( leo, status )
    ...
    select case ( trim(leo%levtype) )
      !~ model levels:
      case ( 'levels', 'halflevels', 'input_levels', 'input_halflevels' )
