        ...
        do i = 1, n
          ...
          if ( density_factor ) then
            ...
            call LE_Data_GetPointer( 'dens', pdens, status, check_units='kg/m3', &
                                 check_lbo=(/0,0,1/), check_ubo=(/nx+1,ny+1,nz+1/) )
            IF_NOTOK_RETURN(status=1)
            ! copy from data variable data shaped (0:nx+1,0:nx+1,1:nz+1):
            bc_west (:,:,ispec) = bc_west (:,:,ispec) + w * pdata(   0,1:ny,1:nz) * factor * pdens(   0,1:ny,1:nz)
            bc_east (:,:,ispec) = bc_east (:,:,ispec) + w * pdata(nx+1,1:ny,1:nz) * factor * pdens(nx+1,1:ny,1:nz)
            bc_south(:,:,ispec) = bc_south(:,:,ispec) + w * pdata(1:nx,   0,1:nz) * factor * pdens(1:nx,   0,1:nz)
            bc_north(:,:,ispec) = bc_north(:,:,ispec) + w * pdata(1:nx,ny+1,1:nz) * factor * pdens(1:nx,ny+1,1:nz)
            caloft  (:,:,ispec) = caloft  (:,:,ispec) + w * pdata(1:nx,1:ny,nz+1) * factor * pdens(1:nx,1:ny,nz+1)

          else

            ! copy from data variable data shaped (0:nx+1,0:nx+1,1:nz+1):
            bc_west (:,:,ispec) = bc_west (:,:,ispec) + w * pdata(   0,1:ny,1:nz) * factor
            bc_east (:,:,ispec) = bc_east (:,:,ispec) + w * pdata(nx+1,1:ny,1:nz) * factor
            bc_south(:,:,ispec) = bc_south(:,:,ispec) + w * pdata(1:nx,   0,1:nz) * factor
            bc_north(:,:,ispec) = bc_north(:,:,ispec) + w * pdata(1:nx,ny+1,1:nz) * factor
            caloft  (:,:,ispec) = caloft  (:,:,ispec) + w * pdata(1:nx,1:ny,nz+1) * factor
