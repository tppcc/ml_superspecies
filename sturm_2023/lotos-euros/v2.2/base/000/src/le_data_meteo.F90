...
contains
  ...
  subroutine LE_Data_Setup_aFS_sfc( mf, tref, tt, rcF, gridtype, mapping, status )
    ...
    if ( status > 0 ) then
      ...
      call mf%Setup_Prepare( nreceive, status )
      IF_NOTOK_RETURN(status=1)
  ...
  subroutine LE_Data_Setup_iFS_3D( mf, tref, t, rcF, gridtype, mapping, status, halflevels )
    ...
    if ( status > 0 ) then
      ...
      do
        ...
        call mf%Setup_Prepare( nreceive, status, reset_levels=(nreset>1) )
        IF_NOTOK_RETURN(status=1)
        ! loop over fields to be received:
        do ireceive = 1, nreceive
          ! get time value:
  ...
  subroutine GetFilename( rcF, rcbase, tref, t, filename, status )
    ...
    integer                ::  dtsec
    character(len=16)      ::  fc00ext, fc12ext, fcext
    character(len=16)      ::  fc00, fc12
    type(TDate)            ::  tf
    real                   ::  rday
    integer                ::  fcday
    type(TDate)            ::  t12
  ...
  subroutine MeteoField_ReadRecordUV_sfc_ugg( tt, filenames, descriptions, &
    ...
    select case ( mapping )
      ...
      !case ( 'interpol' )
      !  ! interpolate to grid points:
      !  call Grid_Convertors%Ugg_Interpol( ugg_in, values_in, ugg, values(:,:,1), status )
      !  IF_NOTOK_RETURN(status=1)
      !!~ nearest neighbour:
      !case ( 'nearest' )
      !  ! interpolate to grid points:
