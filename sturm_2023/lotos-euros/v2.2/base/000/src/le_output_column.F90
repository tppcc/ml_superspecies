...
contains
  ...
  subroutine LE_Output_Column_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    do l = 1, leo%ntr
      ...
      do iz = 1, nz
        ...
        do la = 1, accum_n(itr)
          ! conversion ?
          convfact = 1.0
          if ( accum_ppb_to_ugm3(itr,la) ) convfact = dens(:,:,iz)/xm_air
          ! add:
          pat(1:nx,1:ny) = pat(1:nx,1:ny) + c(:,:,iz,accum_ii(itr,la)) * convfact * accum_ww(itr,la)
        end do
