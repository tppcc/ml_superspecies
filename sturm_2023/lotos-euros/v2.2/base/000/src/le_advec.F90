...
!        integer       ::  off(2)
    ...
    if (status/=0) status_par = status_par + 1
#ifdef with_labeling
    allocate( dummy3(0:nx+1,0:ny+1,0:nz+1), stat=status )
    if (status/=0) status_par = status_par + 1
#endif
      ...
      do istep = 1, nstep_a
        ...
        if ( SA_ispec > 0 ) then
          do SA_ilabel = 1, SA_nlabel
            ...
            dummy3 = SA_frac_adv(:,:,:,SA_ispec,SA_ilabel)          
            call dom%ExchangeHaloX( 1, dummy3, status )
            if ( status /= 0 ) then
              write (gol, '(" Error from exchange of labelfractions halo cells y-direction")' ); call goErr
              status_par = status_par + 1
            end if
            SA_frac_adv(:,:,:,SA_ispec,SA_ilabel) = dummy3
          end do
        end if
#endif
        ...
        if ( SA_ispec > 0 ) then
          do SA_ilabel = 1, SA_nlabel
            ! Exchange fractions from neighbour grids
            dummy3 = SA_frac_adv(:,:,:,SA_ispec,SA_ilabel)          
            call dom%ExchangeHaloY( 1, dummy3, status )
            if ( status /= 0 ) then
              write (gol, '(" Error from exchange of labelfractions halo cells y-direction")' ); call goErr
              status_par = status_par + 1
            end if
