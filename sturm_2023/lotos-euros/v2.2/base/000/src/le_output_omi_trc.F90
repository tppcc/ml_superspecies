...
contains
  ...
  subroutine LE_Output_OMI_TRC_Init( rcfile, status )
    ...
    ! tracer:
    call ReadRc( rcF, 'le.output.omi_trc.tracer', omi_trc%tracer_name, status )
    IF_NOTOK_RETURN(status=1)
    ! search index:
    ...
    ! filename template:
    call ReadRc( rcF, 'le.output.omi_trc.filenames', arch_filenames, status )
    IF_NOTOK_RETURN(status=1)
    
    ! top of tropo sphere to be used with kernel (Pa):
    call ReadRc( rcF, 'le.output.omi_trc.ptropo', omi_trc_ptropo, status )
    IF_NOTOK_RETURN(status=1)
    
    call ReadRC( rcF, 'le.output.omi_trc.top_hp', omi_trc%top_ph_entry, status )
    IF_NOTOK_RETURN(status=1)
    
    call ReadRC( rcF, 'le.output.omi_trc.top_trc', omi_trc%top_trc_entry, status )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine CalcMeas_OMI_TRC( le_vmr_grid, le_vmr_aloft, le2omi_vcd_trop, status )
    ...
    real, intent(out)         ::  le2omi_vcd_trop(maxomi) ! unit 1e15 molec/cm2
    integer, intent(out)      ::  status
  ...
  subroutine leo_PutOut( leo, t, c, status, last )
    ...
    ! storage:
    allocate( le2omi_trc_vcd_trop(1:maxomi) )
