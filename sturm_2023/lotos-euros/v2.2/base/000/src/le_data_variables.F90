    ...
    if ( status == 0 ) then
      ...
      if ( len_trim(V%routine) > 0 ) then
        ...
        select case ( trim(V%routine) )
          ...
          ! select subset of input layers:
          case ( 'SelectLayers' )
            ! arguments: ( *_met )
            call Variables_Calc_SelectLayers( self, trim(varname), V%deps(1:V%ndep), tmid, status )
            IF_NOTOK_RETURN(status=1)	 
          ...
          ! rain rate [m/s] from rain amount [kg/m2]
          case ( 'LWE' )
...
!  end subroutine Variables_Calc_MixlayerHAltBounds
  ...
  subroutine Variables_Calc_HybrideHalflevelPressure( self, varname, args, t, status )
    ...
    select case ( trim(levtype) )
      ...
      case ( 'halflevels' )
        ...
        IF_NOTOK_RETURN(status=1)
      !~ bounds of model levels incl top:
      case ( 'halflevels_top' )
        ! storage:
        allocate( hp(lbo(1):ubo(1),lbo(2):ubo(2),0:nlev+1), stat=status )
        IF_NOTOK_RETURN(status=1)
        ! half-level pressure from surface pressure and hybride coeff:
  ...
  !
  ! Select layers from input fields to target fields:
  !
  !   call Variables_SelectLayers( self, 'x_met', 'x', t, status )
  !

  subroutine Variables_Calc_SelectLayers( self, varname, args, t, status )
    ...
    character(len=*), parameter  ::  rname = mname//'/Variables_Calc_SelectLayers'
  ...
  end subroutine Variables_Calc_SelectLayers
  ...
  ! Monin-Obukhov-Length:
  !   output: monin (grass)
  !   input : lon, z0m, wsurf, tcc, radd, sd
  !
  ...
  subroutine Variables_Calc_MoninObukhovLengthExposure( self, varname, args, t, status )
    ...
    real, parameter             ::  z0_thr = 0.75 ! Stability calculations with z0 above 0.75 are cut-off to 0.75
    ...
    integer, parameter        ::  i_wsurf = 3
    integer, parameter        ::  i_tcc   = 4
    integer, parameter        ::  i_radd  = 5
    integer, parameter        ::  i_sd    = 6
    ...
    do j = lbo(2), ubo(2)
      do i = lbo(1), ubo(1)
        ...
        call exposure_new( radd(i,j,1), wsurf(i,j,1), tcc(i,j,1), coszen(i,j), sd(i,j,1), h_local, stab_class_letter, stab_class, status )
        IF_NOTOK_RETURN(status=1)
        ...
        call CalcMonin_stabclass( stab_class_letter, min(z0m(i,j,1),z0_thr), values(i,j,1), status )
        IF_NOTOK_RETURN(status=1)
