...
contains
  ...
  type(TIncrDate) function dt_times_r4( dt, r )
    ...
    ! fill:
    dt_times_r4 = IncrDate( day  = nint(rday ), &
                            hour = nint(rhour), &
                            min  = nint(rmin ), &
                            sec  = nint(rsec ), &
                            mili = nint(rmili)      )
  ...
  type(TIncrDate) function r4_times_dt( r, dt )
  ...
  type(TIncrDate) function dt_times_r8( dt, r )
    ...
    ! fill:
    dt_times_r8 = IncrDate( day  = nint(rday ), &
                            hour = nint(rhour), &
                            min  = nint(rmin ), &
                            sec  = nint(rsec ), &
                            mili = nint(rmili)      )
  ...
  type(TIncrDate) function r8_times_dt( r, dt )
  ...
  subroutine Get_Surrounding_Interval( t, step, longunits, tt, status )
    ...
    character(len=*), intent(in)  ::  longunits
    ...
    ! offset specified?
    if ( index(longunits,'from') > 0 ) then
      ! copy:
      line = trim(longunits)
      ! split:
      call goReadFromLine( line, units, status, sep=' ' )
      IF_NOTOK_RETURN(status=1)
      call goReadFromLine( line, from, status, sep=' ' )
      IF_NOTOK_RETURN(status=1)
      call goReadFromLine( line, offset, status, sep=' ' )
      IF_NOTOK_RETURN(status=1)
    else
      ! copy:
      units = trim(longunits)
      offset = 0
    end if
  ...
  subroutine goReadFromLine_t( input, t, status, sep, calendar )
    ...
    ! replace seperators:
    call goTranslate( line, '-/:_', ' ', status )
    IF_NOTOK_RETURN(status=1)
