...
! NAME
!
!   LE_Output_DryDep  -  LOTOS-EUROS output of dry deposition labelled fields
!
! HISTORY
!
...
module LE_Output_DryDep_Label
  ...
  public  ::  LE_Output_DryDep_Label_Init, LE_Output_DryDep_Label_Done
  public  ::  LE_Output_DryDep_Label_PutOut


  ! --- const ------------------------------

  character(len=*), parameter   ::  mname = 'LE_Output_DryDep_Label'


  ! --- types ------------------------------

  type T_LE_Output_DryDep_Label
    ! name for this file:
    ...
    ! which process ? index in ex_hour array ?
    character(len=32)           ::  process
    character(len=32)           ::  process_long
    integer                     ::  iex
    ! tracer variables:
    integer                     ::  ntr
    integer, pointer            ::  itr(:)
    ...
    integer                     ::  j1, j2, nj
    real                        ::  westb, southb
  end type T_LE_Output_DryDep_Label
...
contains
  ...
  subroutine LE_Output_DryDep_Label_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    use LE_Budget_DryDepos, only : ex_drydepo, ex_dryemis
    use LE_Data, only : LE_Data_Enable
    use SA_Labeling     , only : ndrydep_label_output_tracer
    use SA_Labeling     , only : labelled_specs_drydep_output_names

    ! --- in/out --------------------------------

    type(T_LE_Output_DryDep_Label), intent(out)   ::  leo
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_DryDep_Init'
    ...
    ! process:
    call ReadRc( rcF, trim(basekey)//'.process', leo%process, status )
    IF_NOTOK_RETURN(status=1)
    ! translate into index parameter:
    select case ( trim(leo%process) )
      case ( 'drydepo' )
        leo%process_long = 'dry_deposition'
        leo%iex          = ex_drydepo
      case ( 'dryemis' )
        leo%process_long = 'dry_emission'
        leo%iex          = ex_dryemis
      case default
        write (gol,'("unsupported process : ",a)') trim(leo%process); call goErr
    ...
    allocate( leo%itr     (ndrydep_label_output_tracer) )
    allocate( leo%itr_glob(ndrydep_label_output_tracer) )
    allocate( leo%name_tr (ndrydep_label_output_tracer) )
    allocate( leo%unit_tr (ndrydep_label_output_tracer) )
    allocate( leo%unitconv(ndrydep_label_output_tracer) )
    allocate( leo%varid_tr(ndrydep_label_output_tracer) )

    ! match tracer names:
    call goMatchValues( tracer_names, labelled_specs_drydep_output_names(1:ndrydep_label_output_tracer), &
  ...
  end subroutine LE_Output_DryDep_Label_Init


  ! ***


  subroutine LE_Output_DryDep_Label_Done( leo, status )
    ...
    type(T_LE_Output_DryDep_Label), intent(inout)   ::  leo
    integer, intent(out)                    ::  status

    ! --- const ----------------------------

    character(len=*), parameter   ::  rname = mname//'/LE_Output_DryDep_Label_Done'
  ...
  end subroutine LE_Output_DryDep_Label_Done


  ! ***


  subroutine LE_Output_DryDep_Label_PutOut( leo, t, status )
    ...
    use LE_Budget_DryDepos, only : nex
    
    ! point to meteo data:
    use LE_Data, only : LE_Data_GetPointer

    use SA_Labeling, only : SA_budget_drydepos_hour
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_DryDep_Label_PutOut'
      ...
      do l = 1, leo%ntr
        ...
        select case ( trim(leo%convention) )
          ...
          case ( 'cf' )
            ...
            varname = trim(leo%name_tr(l))//'_dflux'
            ...
            cf_standard_name = 'tendency_of_atmosphere_mass_content_of_'//trim(cf_tracer_name)//'_due_to_'//trim(leo%process_long)
            ...
            if ( varname == 'pol_b_dflux' .or. varname == 'pol_b' .or. &
                 varname == 'pol_g_dflux' .or. varname == 'pol_g' .or. &
                 varname == 'pol_o_dflux' .or. varname == 'pol_o' ) then
          ...
          case ( 'megapoli' )
            ...
            ! reset to new name and unit ...
            cf_standard_name = trim(leo%process_long)//'_flux_of_'//trim(cf_tracer_name)
            cf_long_name     = trim(cf_standard_name)
            cf_units = 'ug m-2 h-1'
            ! reset to dummy value, not used:
            leo%unitconv(l) = -999.9
    ...
    do l = 1, leo%ntr
      ...
      pat = 0.0
      ! add contributions; 
      pat(1:nx,1:ny,1:SA_nlabel) = SA_budget_drydepos_hour(:,:,itr,:)

      ! unit conversion; 'ex_hour' is 'concentation * height', 
  ...
  end subroutine LE_Output_DryDep_Label_PutOut
    ...
    if ( leo%grads_ctl ) then
      ...
      do l = 1, leo%ntr
        ...
        do SA_ilabel = 1, SA_nlabel
          ...
          write (line,'(a,"_dflux [",a,"]")') trim(leo%name_tr(l)), trim(leo%unit_tr(l))
          ! set dimension description t, labelnr, z,y,x
          write (dim_descr, '("t, ",i2,",y,x")') SA_ilabel-1  
          ! add variable description:
          call GrADS_Ctl_Var( ctl, trim(leo%name_tr(l))//'_dflux', 0, trim(dim_descr), trim(line), status, label_name = SA_Short_Label_Names(SA_ilabel) )
          IF_NOTOK_RETURN(status=1)
        end do
      end do
...
end module LE_Output_DryDep_Label
