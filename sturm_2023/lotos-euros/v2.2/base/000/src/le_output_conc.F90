...
contains
  ...
  subroutine LE_Output_Conc_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    select case ( trim(leo%levtype) )
      ...
      case ( 'heights', 'elevations' )
        ...
        call ReadRc( rcF, trim(basekey)//'.height_dist', leo%height_dist, status)
        IF_NOTOK_RETURN(status=1)
      !~ unknown
      case default
    ...
    ! write halo cells ?
    call ReadRc( rcF, trim(basekey)//'.halo', leo%halo, status )
    ...
    call ReadRc( rcF, trim(basekey)//'.addheight', leo%addheight, status )
    IF_ERROR_RETURN(status=1)

    ! write layer thickness field to conc file ?
    call ReadRc( rcF, trim(basekey)//'.addthickness', leo%addthickness, status )
    IF_ERROR_RETURN(status=1)
  ...
  subroutine LE_Output_Conc_PutOut( leo, t, c, cg, status )
    ...
    allocate( field(leo%i1-leo%nhx:leo%i2+leo%nhx,leo%j1-leo%nhy:leo%j2+leo%nhy,0:nz+1) )
    ! 2d field:
...
!#endif
    ...
    if ( (trim(leo%levtype) == 'heights') .or. &
      ...
      ! half level heights:
      allocate( hh(1-leo%nhx:nx+leo%nhx,1-leo%nhy:ny+leo%nhy,0:nz+1) )
      ! lowest is orography:
      ...
      if (trim(leo%height_dist) == 'interp') then
        ...
        ! dummy for aloft ...
        hh(1:nx,1:ny,nz+1) = oro(1:nx,1:ny,1) + h_m(1:nx,1:ny,nz) + 1000.0   ! 1000 m above top
        ...
        ! dummy for aloft
        hh(1:nx,1:ny,nz+1) = oro(1:nx,1:ny,1) + h_m(1:nx,1:ny,nz) + 2000.0  ! 2000 m above top
      else
      ...
      do ilev = 0, nz+1
        ...
        !~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~~
          ...
          do la = 1, accum_n(itr)
            ! conversion needed ?
            convfact = 1.0
            if ( accum_ppb_to_ugm3(itr,la) ) convfact = dens(:,:,nz)/xm_air
            ! core:
            pat(leo%i1:leo%i2,leo%j1:leo%j2) = pat(leo%i1:leo%i2,leo%j1:leo%j2) &
                    + caloft(leo%i1:leo%i2,leo%j1:leo%j2,accum_ii(itr,la)) &
                       * convfact(leo%i1:leo%i2,leo%j1:leo%j2) * accum_ww(itr,la)
          end do
