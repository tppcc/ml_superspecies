...
! NAME
!
!   LE_Output_WetDep  -  LOTOS-EUROS output of concentration fields
!
! HISTORY
!
...
module LE_Output_WetDep
  ...
  public  ::  LE_Output_WetDep_Init, LE_Output_WetDep_Done
  public  ::  LE_Output_WetDep_PutOut


  ! --- const ------------------------------

  character(len=*), parameter   ::  mname = 'LE_Output_WetDep'


  ! --- types ------------------------------

  type T_LE_Output_WetDep
    ! name for this file:
...
contains
  ...
  subroutine LE_Output_WetDep_Init( leo, rcfile, rckey, typ, name, state, status )
    ...
    type(T_LE_Output_WetDep), intent(out)   ::  leo
    ...
    character(len=*), parameter   ::  rname = mname//'/LE_Output_WetDep_Init'
  ...
  end subroutine LE_Output_WetDep_Init


  ! ***


  subroutine LE_Output_WetDep_Done( leo, status )
    ...
    type(T_LE_Output_WetDep), intent(inout)   ::  leo
    integer, intent(out)                    ::  status

    ! --- const ----------------------------

    character(len=*), parameter   ::  rname = mname//'/LE_Output_WetDep_Done'
  ...
  end subroutine LE_Output_WetDep_Done


  ! ***


  subroutine LE_Output_WetDep_PutOut( leo, t, cwet, status )
    ...
    real, intent(in)                        ::  cwet(nx,ny,nz,nspec)
    integer, intent(out)                    ::  status

    ! --- const ---------------------------------

    character(len=*), parameter   ::  rname = mname//'/LE_Output_WetDep_PutOut'
    ...
    allocate( convfact(1:nx,1:ny,1:nz) )
    ...
    do l = 1, leo%ntr
      ...
      do la = 1, accum_n(itr)
        ...
        ! add:
        pat(1:nx,1:ny) = pat(1:nx,1:ny) + sum(cwet(:,:,:,accum_ii(itr,la)) * convfact * accum_ww(itr,la),dim=3)
      end do
      ...
      select case ( trim(specunit(itr))//' m -> '//trim(leo%unit_tr(l)) )
        ...
        case ( 'ppb m -> ug m-2 h-1' )
          ! Not accumulated tracer, then conversion factor used above is equal to 1, convert for each layer
          if ( accum_n(itr) == 1 ) then
            !   ug/m2      =       
            pat(1:nx,1:ny) = &
             !        ppb m      *((mole tr/mole air)/ppb) * (kg tr/mole tr) * (kg air/m3 air)  / (kg air/mole air) * ug/kg
             sum(cwet(:,:,:,itr) *         1e-9            * specmolm(itr)   * dens(1:nx,1:ny,:)/      xm_air       * 1e9 , dim=3 )
          else
            write( gol, '("Error in converting deposition flux for accumulated tracer with unit ppb")') ; call GoErr
            write( gol, '("Tracer: ", a)') trim(specname(itr)) ; call GoErr
            TRACEBACK;status=1;return
          end if
          ! per hour:
          pat = pat / real(leo%dhour)  ! ug/m2/hr
        case ( 'ppb m -> kg m-2 s-1' )
          ...
          if ( accum_n(itr) == 1 ) then
             ! kg/m2  
            pat(1:nx,1:ny) = &
             !         ppb  m    * ((mole tr/mole air)/ppb) * (kg tr/mole tr) *   (kg air/m3 air)    / (kg air/mole air)
             sum(cwet(:,:,:,itr) *       1e-9               *  specmolm(itr)  * dens(1:nx,1:ny,1:nz) /     xm_air      , dim=3) 
          else
            write( gol, '("Error in converting deposition flux for accumulated tracer with unit ppb")') ; call GoErr
            write( gol, '("Tracer: ", a)') trim(specname(itr)) ; call GoErr
            TRACEBACK;status=1;return
  ...
  end subroutine LE_Output_WetDep_PutOut
  ...
  subroutine Write_GrADS_Ctl( leo, status )
    ...
    if ( leo%grads_ctl ) then
      ...
      do l = 1, leo%ntr
        ...
        call GrADS_Ctl_Var( ctl, trim(leo%name_tr(l))//'_wflux', 0, 't,y,x', trim(line), status )
        IF_NOTOK_RETURN(status=1)
      end do
