...
contains
  ...
  subroutine CSV_Init( self, fname, keys, status )
    ...
    call goVarValue( keys, ';', 'comp', '=', key, status, verbose=.true. )
    IF_NOTOK_RETURN(status=1)
    ...
    call goVarValue( keys, ';', 'filter_time', '=', self%filter_time, status, verbose=.true. )
    IF_NOTOK_RETURN(status=1)
    ! enabled?
    ...
    call goVarValue( keys, ';', 'filter_comp', '=', self%filter_comp, status, verbose=.true. )
    IF_NOTOK_RETURN(status=1)
  ...
  subroutine CSV_ReadRecord( self, code, comp, value, units, status, t )
    ...
    use GO, only : TDate, NewDate, operator(/=)
    ...
    do
      ...
      if ( len_trim(self%filter_comp) > 0 ) then
        ! skip if not the same:
        if ( trim(comp) /= trim(self%filter_comp) ) cycle
      end if
