...
!-------------------------------------------------------------------
subroutine DryDepos_Gas_DEPAC(compnam, day_of_year, lat, t, ust, glrad, sinphi, rh, lai, sai, nwet, lu, iratns, rc_tot, ccomp_tot, hlaw, react, &
                   p, tsea, smi, &
...
real, optional   , intent(in)  :: hlaw            ! effective Henry's law constant (M atm-1)
real, optional   , intent(in)  :: react           ! reactivity (-)
...
select case ( trim(compnam) )
  ...
  case default
    write(*,*) ' Unknown component name: ', trim(compnam)
    stop
...
if (.not. ready) then
   ...
   call rc_gsoil_eff( icmp, compnam, lu, sai, ust, nwet, t, &
                        hlaw, react, icmp_o3, icmp_so2, gsoil_eff )
...
if ( ccomp_tot == 0.0 ) then
  ...
  rc_eff = ((ra + rb)*ccomp_tot + rc_tot*catm)/(catm-ccomp_tot)
   ...
   rc_eff = -999.
   print *, 'This should not be possible--- rc_comp_point_rc_eff in le_dry_depos_gas_depac.F90'
   print *, 'rc_tot: ', rc_tot
   print *, 'ccompt_tot: ', ccomp_tot
   print *, 'catm: ', catm   
   stop
endif
