...
!    ! --- external ------------------------------------------------
!    
!    ! compiler intrinsics, but not part of standard Fortran:
!    external  ::  System
!    
!    ! --- local --------------------------------------------------
!    
...
!        case default
!          write (gol,'("  unknown iErrNo ",i6)') errno; call goErr
!      end select
...
!  end subroutine goSystem
  ...
  subroutine goExit( status )
    ...
    !         Exit Subroutine
    !
    ! This is not allowed in the Fortran 2003 standard ...
    !call Exit( status )
    ! Instead, jus return with the error status if necessary:
    if ( status == 0 ) then
      stop
    else
      stop 1
  ...
  subroutine goGetArg( pos, value, status )
    ...
    !
!#ifdef __GNUC____GNUC_MINOR____GNUC_PATCHLEVEL__ == 435
!    ! up to version 4.3.5 :
!    call Get_Command_Argument( pos, value )
!    ! no status returned ...
!    status = 0
