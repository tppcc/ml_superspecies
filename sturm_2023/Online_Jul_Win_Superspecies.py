...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug  6 10:33:37 2021
...
# For winter optimized superspecies
nc = get_data(start_date = start, stop_date = stop,
              folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
              file_prefix = "/LE_jul_conc-sfc_")
nc_ML = get_data(start_date = start, stop_date =  stop,
                  folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-ML/jul/output",
                  file_prefix = "/LE_jul_conc-sfc_")
nc_met = get_data(start_date = start, stop_date = stop,
                  folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
...
# For winter optimized superspecies, composition matrices (Strategy 3)
# nc = get_data(start_date = start, stop_date = stop,
#               folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
#               file_prefix = "/LE_jul_conc-sfc_")
# nc_ML = get_data(start_date = start, stop_date =  stop,
#                   folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-ML/jul_composition_matrices/output",
#                   file_prefix = "/LE_jul_conc-sfc_")
# nc_met = get_data(start_date = start, stop_date = stop,
#                   folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
#                   file_prefix = "/LE_jul_meteo-sfc_")
...
# # For summer optimized superspecies 
# nc = get_data(start_date = start, stop_date = stop,
#               folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
#               file_prefix = "/LE_jul_conc-sfc_")
# nc_ML = get_data(start_date = start, stop_date =  stop,
#                  folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-ML/jul_summer_superspecies/output",
#                  file_prefix = "/LE_jul_conc-sfc_")
# nc_met = get_data(start_date = start, stop_date = stop,
#                   folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-CONTROL/jul/output",
#                   file_prefix = "/LE_jul_meteo-sfc_")
...
# # 
# Get longitude and latitude for different gridcells
...
# tuebingen (southern german forest) latitude longitude
...
# Determine components of TOA: how are they in the summer?
bSOA_frac = np.mean(np.sum(B[0:6,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)) + 1e-9)
aSOA_frac = np.mean(np.sum(A[0:6,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)) + 1e-9)
siSOA_frac = np.mean(np.sum(SI[0:8,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)) + 1e-9)
POA_frac = np.mean(np.sum(P[0:9,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)) + 1e-9)
...
plot_TOA(average_TOA,average_TOA_ML, date = ", July 25 - August 1 2018")
...
bias_A = np.mean(A_ML-A)
bias_B = np.mean(B_ML-B)
bias_SI = np.mean(SI_ML-SI)
bias_P = np.mean(P_ML-P)

normalized_bias_A = np.sum(A_ML-A)/np.sum(A)
normalized_bias_B = np.sum(B_ML-B)/np.sum(B)
normalized_bias_SI = np.sum(SI_ML-SI)/np.sum(SI)
normalized_bias_A = np.sum(P_ML-P)/np.sum(P)

bias_bSOA = np.mean(B_ML[0:6,:]-B[0:6,:])
...
vbses = (B_reshaped,B_ML_reshaped)
titles = ("Cabauw bSOA, july sometime 20:00, LOTOS EUROS", "LOTOS-EUROS with online")
ax1 = plot_multiple_vbs_withconc(vbses,44,coords[0],coords[1],titles,("g","maroon"))

coords = (caba_lati,caba_long) 
vbses = (SI_reshaped,SI_ML_reshaped)
titles = ("Cabauw siSOA, july sometime 20:00, LOTOS EUROS", "LOTOS-EUROS with online")
ax1 = plot_multiple_vbs_withconc(vbses,44,coords[0],coords[1],titles,("g","maroon"))


#%%

# What about bSOA near the Schoenbuch?coords = (caba_lati,caba_long) 
Bsoa_tub_diff = B_ML_reshaped[0:6,:,0,tubi_lati,tubi_long] - B_reshaped[0:6,:,0,tubi_lati,tubi_long]
index_max_tub = np.unravel_index(Bsoa_tub_diff.argmax(), Bsoa_tub_diff.shape)
# Note: from now on, hardcoded assuming that index_max_tub[1] = 173

coords = (tubi_lati,tubi_long) 
vbses = (B_reshaped,B_ML_reshaped)
titles = ("Schönbuch bSOA, July 30 20:00", "LOTOS-EUROS with online ML")
ax1 = plot_multiple_vbs_withconc(vbses,index_max_tub[1]-9,coords[0],coords[1],titles,("g","maroon"),ymax = 30)
plt.savefig('figs/tubi_8pm.pdf', format='pdf')

# What about bSOA near the Schoenbuch?coords = (caba_lati,caba_long) 
coords = (tubi_lati,tubi_long) 
vbses = (B_reshaped,B_ML_reshaped)
titles = ("Schönbuch bSOA, July 31 05:00", "LOTOS-EUROS with online ML")
ax1 = plot_multiple_vbs_withconc(vbses,index_max_tub[1],coords[0],coords[1],titles,("g","maroon"),ymax = 30)
plt.savefig('figs/tubi_5am.pdf', format='pdf')

coords = (tubi_lati,tubi_long) 
vbses = (B_reshaped,B_ML_reshaped)
titles = ("Schönbuch bSOA, July 31 09:00", "LOTOS-EUROS with online ML")
ax1 = plot_multiple_vbs_withconc(vbses,index_max_tub[1]+4,coords[0],coords[1],titles,("g","maroon"),ymax = 30)
plt.savefig('figs/tubi_9am.pdf', format='pdf')
...
# Plotting time-averaged TOA and bias on domain
date = ", Jul 29 - Aug 1 2018"
figLE = QuickMap( average_TOA[0,0,:,:], xx = lons, yy = lats,
                         bmp=dict(countries = 'True', resolution = 'i'), 
                         cbar=dict( label='Average TOA (ug/m3)'),
                         vmin=0, vmax=np.max([np.max(average_TOA),np.max(average_TOA_ML)]),
                         figsize=(6,5), title = 'Control Run' + date
                   )
plt.savefig('figs/online_july_control.pdf', format='pdf')

figNMF = QuickMap( (average_TOA_ML[0,0,:,:]),  domain = [minlon, maxlon, minlat, maxlat],
                         bmp=dict(countries = 'True', resolution = 'i'), 
                         cbar=dict( label='Average TOA (ug/m3)'),
                         vmin=0, vmax=np.max([np.max(average_TOA),np.max(average_TOA_ML)]),
                         figsize=(6,5), title = 'Winter-optimized Superspecies Run' +date
                      )
plt.savefig('figs/online_july_winsup_superspecies.pdf', format='pdf')
...
figbias = QuickMap( average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:],
                  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         vmin=-np.max(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:]), 
                         vmax=np.max(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:]),
                         figsize=(6,5), title = "Winter-optimized Superspecies Run, Bias"
                      )
plt.savefig('figs/online_july_winsup_bias.pdf', format='pdf')
...
figrelbias = QuickMap( 100*(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:] )/average_TOA[0,0,:,:] ,
                         ...
                         cbar=dict( label='Relative bias'),
                         vmin=-100, vmax=100, cmap=dict( colors=['blue', 'white','red'], ncolor=500 ),
                         figsize=(6,5), title = 'Winter-optimized Superspecies Run, Normalized Bias'
                      )
plt.savefig('figs/online_july_winsup_relbias.pdf', format='pdf')


plot_online_timeseries(tubi_lati,tubi_long,TOA,TOA_ML,site="Schönbuch with Winter Superspecies",timerange="July 29 - August 1")
plt.savefig('figs/tubi_timeseries_winsup.pdf', format='pdf')

#%% Representative plots for each VBS class for chapter 2
time_example = 28
# plot_vbs(P_reshaped,time_example ,caba_lati,caba_long,title = "POA VBS",bincolor = "sienna")
# plt.savefig('figs/POA_example.pdf', format='pdf')
plot_vbs(SI_reshaped,time_example ,tubi_lati,tubi_long,title="siSOA VBS",bincolor ="darkorange")
plt.savefig('figs/siSOA_example.pdf', format='pdf')
plot_vbs(A_reshaped,time_example ,tubi_lati,tubi_long,title="aVOC VBS",bincolor="turquoise")
plt.savefig('figs/aSOA_example.pdf', format='pdf')
plot_vbs(B_reshaped,time_example ,tubi_lati,tubi_long,title="bVOC VBS",bincolor="olivedrab")
plt.savefig('figs/bSOA_example.pdf', format='pdf')

#%% Get giffy

# def make_gif(nc,TOA_reshaped,title="TOA",gifname='movie'):
#     base_date = datetime(2018, 1, 1) 
#     files = []
#     for i in range(0,24*4):
        
#         d = base_date + timedelta(seconds=(nc["time"][i])) 
        
#         fig = QuickMap( (RMSE[i,0,:,:]), domain =[minlon,maxlon,minlat,maxlat], 
#                          bmp=dict(countries = 'True', resolution = 'i'), 
#                          cbar=dict( label=label),
#                          vmin=0, vmax=np.max(RMSE),
#                          figsize=(6,5), title = str(nc['time'][0].values)[0:16]
#                          )
