...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 30 12:37:27 2021
...
figNMF = QuickMap( (average_TOA_ML[0,0,:,:]),  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         cbar=dict( label='Average TOA (ug/m3)'),
                         vmin=0, vmax=np.max([np.max(average_TOA),np.max(average_TOA_ML)]),
                         figsize=(6,5), title = 'LE February Run with Summer Superspecies' +date
                      )
plt.savefig('figs/online_feb_sumsup_superspecies.pdf', format='pdf')
...
figbias = QuickMap( average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:],
                  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         figsize=(6,5), title = "LE February Run with Summer Superspecies, Bias"
                      )
plt.savefig('figs/online_feb_sumsup_bias.pdf', format='pdf')
...
figrelbias = QuickMap( 100*(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:] )/average_TOA[0,0,:,:] ,
                         ...
                         cbar=dict( label='Relative bias'),
                         vmin=-100, vmax=100, cmap=dict( colors=['blue', 'white','red'], ncolor=500 ),
                         figsize=(6,5), title = 'LE February Run with Summer Superspecies, Normalized Bias'
                      )
plt.savefig('figs/online_feb_sumsup_relbias.pdf', format='pdf')
