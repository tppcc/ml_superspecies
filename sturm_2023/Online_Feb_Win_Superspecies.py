...
# -*- coding: utf-8 -*-
"""
Created on Fri Aug 20 13:47:39 2021
...
def plot_online_site(lati,long,site, TOA, TOA_ML, timerange= ""):
    # This function plots timeseries of TOA given additionally latitude, longitude, and the name of a site.
    time = np.arange(0,9,1/24)
    fig, timeseries = subplots()
...
# nc_ML = get_data(start_date = start, stop_date =  stop,
#                   folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-ML/feb_summer_superspecies/output",
#                   file_prefix = "/LE_feb_conc-sfc_")
# nc_met_ML = get_data(start_date = start, stop_date = stop,
#                         folder = "/Scratch/sturmpo/projects/LOTOS-EUROS-ML/feb_summer_superspecies/output",
#                         file_prefix = "/LE_feb_meteo-sfc_")
...
# Determine components of TOA
bSOA_frac = np.mean(np.sum(B[0:6,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)))
aSOA_frac = np.mean(np.sum(A[0:6,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)))
siSOA_frac = np.mean(np.sum(SI[0:8,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)))
POA_frac = np.mean(np.sum(P[0:9,:],axis = 0)/(np.sum(A[0:6,:],axis = 0) + np.sum(B[0:6,:],axis = 0) + np.sum(SI[0:8,:],axis = 0) + np.sum(P[0:9,:],axis = 0)))
...
figLE = QuickMap( average_TOA[0,0,:,:], xx = lons, yy = lats,
                         ...
                         figsize=(6,5), title = 'LE Winter Control Run' + date
                   )
plt.savefig('figs/online_feb_control.pdf', format='pdf')

figNMF = QuickMap( (average_TOA_ML[0,0,:,:]),  domain = [minlon, maxlon, minlat, maxlat],
                         bmp=dict(countries = 'True', resolution = 'i'), 
                         cbar=dict( label='Average TOA (ug/m3)'),
                         vmin=0, vmax=np.max([np.max(average_TOA),np.max(average_TOA_ML)]),
                         figsize=(6,5), title = 'LE February Run with Winter Superspecies' +date
                      )
plt.savefig('figs/online_feb_winsup_superspecies.pdf', format='pdf')
...
figbias = QuickMap( average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:],
                  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         figsize=(6,5), title = "LE February Run with Winter Superspecies, Bias"
                      )
plt.savefig('figs/online_feb_winsup_bias.pdf', format='pdf')
...
figrelbias = QuickMap( 100*(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:] )/average_TOA[0,0,:,:] ,
                         ...
                         cbar=dict( label='Relative bias'),
                         vmin=-100, vmax=100, cmap=dict( colors=['blue', 'white','red'], ncolor=500 ),
                         figsize=(6,5), title = 'LE February Run with Winter Superspecies, Normalized Bias'
                      )
plt.savefig('figs/online_feb_winsup_relbias.pdf', format='pdf')
...
#%% Cabauw and Mace Head
plot_online_site(caba_lati,caba_long,"Cabauw", TOA, TOA_ML, timerange= "Feb 25-28")
plot_online_site(mace_lati,mace_long,"Mace Head", TOA, TOA_ML, timerange= "Feb 25-28")