...
# -*- coding: utf-8 -*-
"""
Created on Wed Jun 23 10:03:56 2021
...
normalized_bias_aSOA = np.sum(A_ML[0:6,:]-A[0:6,:])/np.sum(A[0:6,:])
normalized_bias_bSOA = np.sum(B_ML[0:6,:]-B[0:6,:])/np.sum(B[0:6,:])
normalized_bias_siSOA = np.sum(SI_ML[0:8,:]-SI[0:8,:])/np.sum(SI[0:8,:])
normalized_bias_POA = np.sum(P_ML[0:9,:]-P[0:9,:])/np.sum(P[0:9,:])

normalized_RMSE_A = RMSE_A/np.mean(A)
normalized_RMSE_B = RMSE_B/np.mean(B)
normalized_RMSE_SI = RMSE_SI/np.mean(SI)
normalized_RMSE_P = RMSE_P/np.mean(P)
...
max_bSOA_overestimation = np.max(np.sum(B_ML[0:6,:],axis=0)-np.sum(B[0:6,:],axis=0))
max_siSOA_overestimation = np.max(np.sum(SI_ML[0:8,:],axis=0)-np.sum(SI[0:8,:],axis=0))
...
figLE = QuickMap( average_TOA[0,0,:,:], xx = lons, yy = lats,
                         ...
                         figsize=(6,5), title = 'LE July Control Run' + date
                   )
...
figNMF = QuickMap( (average_TOA_ML[0,0,:,:]),  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         cbar=dict( label='Average TOA (ug/m3)'),
                         vmin=0, vmax=np.max([np.max(average_TOA),np.max(average_TOA_ML)]),
                         figsize=(6,5), title = 'LE July Run with Summer Superspecies' +date
                      )
plt.savefig('figs/online_july_sumsup_superspecies.pdf', format='pdf')
...
figbias = QuickMap( average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:],
                  domain = [minlon, maxlon, minlat, maxlat],
                         ...
                         figsize=(6,5), title = "LE July Run with Summer Superspecies, Bias"
                      )
plt.savefig('figs/online_july_sumsup_bias.pdf', format='pdf')
...
figrelbias = QuickMap( 100*(average_TOA_ML[0,0,:,:] - average_TOA[0,0,:,:] )/average_TOA[0,0,:,:] ,
                         ...
                         cbar=dict( label='Relative bias'),
                         vmin=-100, vmax=100, cmap=dict( colors=['blue', 'white','red'], ncolor=500 ),
                         figsize=(6,5), title = 'LE July Run with Summer Superspecies, Normalized Bias'
                      )
plt.savefig('figs/online_july_sumsup_relbias.pdf', format='pdf')

plot_online_timeseries(tubi_lati,tubi_long,TOA,TOA_ML,site="Schönbuch with Summer Superspecies",timerange="July 29 - August 1")
plt.savefig('figs/tubi_timeseries_sumsup.pdf', format='pdf')
...
# plot_vbs(SI_reshaped,time_example ,caba_lati,caba_long,title="siSOA VBS",bincolor ="darkorange")
# plt.savefig('figs/siSOA_example.pdf', format='pdf')
# plot_vbs(A_reshaped,time_example ,caba_lati,caba_long,title="aVOC VBS",bincolor="aquamarine")
# plt.savefig('figs/aSOA_example.pdf', format='pdf')
# plot_vbs(B_reshaped,time_example ,caba_lati,caba_long,title="bVOC VBS",bincolor="olivedrab")
